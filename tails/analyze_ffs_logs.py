...
from credit.rare_events.hurricane_genesis_ffs import FFSLogger
...
def plot_pathway_images(pathway: list, ic_dir: Path, output_path: Path, target_label: str = 'B'):
    """Create a figure showing PNG images for each step in the pathway."""
    ...
    for i, step in enumerate(pathway):
        ...
        if config_name is None:
            axes[i].text(0.5, 0.5, "Incomplete pathway\n(parent config missing)", 
                        ha='center', va='center', transform=axes[i].transAxes,
                        fontsize=12, color='red')
            axes[i].axis('off')
            continue
        
        # Hierarchical directory structure: IC_DIR/flux/ or IC_DIR/{interface_idx}/
        if config_name.startswith('lambda0_'):
            ...
            # Extract interface number from lambda1_config_* -> '1'
...
# def load_all_logs(log_dir: Path) -> list:
#     """Load and merge all log files from all workers and ranks."""
#     all_entries = []
    
#     pattern = 'ffs_log_world*_rank*_worker*.jsonl'
...
#     # Initialize logger - USES BASE output_dir for analyzer logs
#     logger = FFSLogger(
...
#         # NEW STRUCTURE: Everything under IC directory
#         ic_dir = output_dir / time_label
#         flux_dir = ic_dir / 'flux'
#         log_dir = ic_dir / 'logs'  # LOGS ARE NOW IN IC DIRECTORY
        
#         logger.info(f"Loading log files from {log_dir}")
...
#             pathway_figs_dir = output_dir / 'pathway_figures' / 'stateB'
#             pathway_figs_dir.mkdir(parents=True, exist_ok=True)
...
#             if target_configs:
#                 pathway_figs_dir = output_dir / 'pathway_figures' / f'lambda{target_lambda_label}'
#                 pathway_figs_dir.mkdir(parents=True, exist_ok=True)
