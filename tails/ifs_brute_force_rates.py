"""
Compute brute force hurricane genesis rates from IFS ensemble forecasts.
...
adaptive spatial tracking with jump detection to maintain trajectory continuity.
...
class IFSBruteForceRateEstimator:
    ...
    adaptive spatial search and trajectory continuity validation.
    """
    ...
    def load_data(self, forecast_times: list = None):
        ...
        if forecast_times is not None and len(forecast_times) > 0:
            ...
            start_str, end_str = forecast_times[0]
            start_time = pd.to_datetime(start_str)
            end_time = pd.to_datetime(end_str)
            
            print(f"Filtering to time range: {start_str} to {end_str}")
            self.ds_filtered = self.ds.sel(time=slice(start_time, end_time))
        else:
    ...
    def process_single_trajectory(self,
        ...
        with adaptive tracking and jump detection.
    ...
    def _process_single_init_parallel(self, init_idx: int, init_time, n_inits: int) -> Dict:
        ...
        print(f"Init {init_idx+1}/{n_inits} ({init_time}): B:{n_success} ({rate_per_day:.6f}/day), {interface_str}{track_str}")
    ...
    def compute_brute_force_rates(self,
        ...
        Tracks crossings at each interface with adaptive spatial tracking.
        ...
        print(f"Lead times: {len(self.ds_filtered.prediction_timedelta)} (full forecast)")
...
def main():
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Forecast period: {ffs_config['forecast_times'][0][0]} to {ffs_config['forecast_times'][0][1]}\n")
...
# Uses TOBAC tracking to detect and track low pressure systems, counting
# how many trajectories successfully develop into hurricanes.
...
#     Uses TOBAC tracking to identify and follow individual systems.
#     """
...
#                            mslp_hpa: np.ndarray,
#                            lats: np.ndarray,
#                            lons: np.ndarray) -> Tuple[float, Optional[Tuple[float, float]]]:
#         """
#         Extract minimum MSLP using TOBAC feature detection.
        
#         Returns:
#             (min_mslp, (lat, lon)) or (min_mslp, None) if tobac fails
#         """
#         if not TOBAC_AVAILABLE:
#             return self._extract_mslp_fallback(mslp_hpa, lats, lons)
...
#         # Sort longitudes for tobac (it requires monotonic coordinates)
#         mslp_tobac = mslp_basin.copy()
#         lons_tobac = lons_180.copy()
...
#                 min_row = int(strongest['hdim_1'])
#                 min_col = int(strongest['hdim_2'])
                
#                 min_mslp = float(mslp_tobac[min_row, min_col])
                
#                 if not np.isnan(min_mslp):
#                     # Get location
#                     min_lat = lats[min_row]
#                     min_lon = lons_tobac[min_col]
#                     return min_mslp, (min_lat, min_lon)
...
#         else:
#             mslp_init = mslp_init_raw
#         mslp_init_min, loc_init = self.extract_mslp_tobac(mslp_init, lats, lons)
...
#             min_mslp, location = self.extract_mslp_tobac(mslp_hpa, lats, lons)
...
#             'min_mslp': np.min(mslp_trajectory),
#             'success_lead_time': None,
#             'success_lead_hours': max_lead_hours  # Store full forecast length for rate calc
#         }
#         # Add interface crossings (boolean and times)
#         for i, crossed in interface_crossings.items():
#             result[f'crossed_interface_{i}'] = crossed
#             result[f'crossing_time_interface_{i}'] = interface_crossing_times[i] if crossed else max_lead_hours
#         return result
...
#         # FIXED: Compute actual rates using total simulation time
#         total_sim_days = traj_df['success_lead_hours'].sum() / 24.0
#         rate_per_day = n_success / total_sim_days if total_sim_days > 0 else 0.0
...
#             'rate_per_day': rate_per_day,
#             'rate_from_A_per_day': rate_from_A_per_day,
#             'prob_overall': prob_overall,  # Keep for reference
#             'prob_from_A': prob_from_A,    # Keep for reference
#             'mean_passage_time_hours': mean_lead
#         }
...
#             init_result[f'n_crossed_lambda{i}'] = n_crossed
#             init_result[f'rate_lambda{i}_per_day'] = rate_lambda_i
#             init_result[f'prob_lambda{i}'] = n_crossed / n_members  # Keep probability too
        
#         # Progress message
#         interface_str = ', '.join([f"λ{i}:{init_result[f'n_crossed_lambda{i}']}" 
#                                   for i in range(len(self.interfaces))])
#         print(f"Init {init_idx+1}/{n_inits} ({init_time}): B:{n_success} ({rate_per_day:.6f}/day), {interface_str}")
...
#         """
#         Compute brute force rates per initialization time.
#         Tracks crossings at each interface.
...
#             # Compute pooled rate
#             total_sim_days_interface = 0
#             for _, row in df.iterrows():
#                 # Sum all crossing times for this interface
#                 init_df = pd.DataFrame([r for r in init_results if r['init_time'] == row['init_time']])
#                 # This is already aggregated, so we use the parent calculation
#                 pass
