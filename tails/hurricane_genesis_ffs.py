...
class HurricaneGenesisFFS:
    ...
    def __init__(self,
        ...
        # Interior land mask: erode coastlines by ~100 km (1 grid cell at 1°)
        # so coastal points and small islands are NOT masked
        land_binary = self.land_sea_mask > 0.5
        self.interior_land_mask = binary_erosion(land_binary, iterations=1)
    ...
    def extract_mslp(self, y_phys, batch=None, forecast_step=None,
                    parent_location=None, mode="flux", stop_tracking_new=False):
        ...
        if len(self.tracked_storms) > 0:
            for storm_id, storm_info in self.tracked_storms.items():
                ...
                min_lon = float(lons[j])
                min_mslp = float(mslp_hpa[i, j])

                if min_lat < 12.0:  # Low latitude -- our MSLP is less than ideal and sometimes we get fixed low pressure points over South America
        ...
        if not stop_tracking_new:
            exclude_centers = [storm_info['location'] for storm_info in self.tracked_storms.values()]
    ...
    def _find_local_mslp_minima(self, mslp_hpa, lats, lons, basin_mask,
                                exclude_centers=None, exclude_radius_deg=12.0,
                                smooth_sigma=1.5):
        """Find local MSLP minima - returns ALL minima."""
        if basin_mask is not None:
            field = np.where(basin_mask, mslp_hpa, np.nan)  # ← Uses basin mask
        else:
            field = mslp_hpa.copy()
        
        # Support multiple exclusion centers
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                mslp, feat_idx, location = self.extract_mslp(
                    y_with_mslp, batch, step, previous_location, mode, stop_tracking_new
                )
                
                trajectory_mslp.append(mslp)

                # Check if tracked storms have become extratropical (FLUX MODE ONLY)
                if mode == 'flux' and self.use_cps and len(self.tracked_storms) > 0:
                    for storm_id in list(self.tracked_storms.keys()):
                        storm = self.tracked_storms[storm_id]
                        storm_lat, storm_lon = storm['location']
                        
                        # Only check storms at high latitudes or far east
                        if storm_lat > 50.0 or storm_lon > -10.0:
                            try:
                                if pressure_interp is None:
                                    print("  → Computing pressure interp for ET check...")
                                ...
                                if is_ET:
                                    print(f"  → Storm {storm_id} became {cps['phase']} at ({storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - removing")
                                    del self.tracked_storms[storm_id]
                ...
                if mode == 'shoot' and location:
                    ...
                    if storm_lat > 60.0:
                        ...
                        # (e.g., storms heading into South America are non-physical)
                        # shortcoming of our storm-tracker. 
                        # In the future we may want to implement a more robust tracking approach that can handle this better, but for now we'll just reject these trajectories.
                        lats = self.latlons.latitude.values
                        lons = np.where(
                            self.latlons.longitude.values > 180,
                            self.latlons.longitude.values - 360,
                    ...
                    for storm_id in list(self.tracked_storms.keys()):
                        ...
                        if not storm['saved'] and storm['mslp'] < lambda_0:
                            ...
                            j_max = min(mslp_hpa.shape[1], lon_idx + 2)
                             ...
                             # Hispaniola region — defer λ₀ decision until storm moves away
                            in_hispaniola = abs(storm_lat - 18.0) < 2.0 and abs(storm_lon - (-70.0)) < 3.0
                            
                            if in_hispaniola:
                                if 'deferred_lambda0_step' not in storm:
                                    storm['deferred_lambda0_step'] = step
                                    storm['deferred_lambda0_mslp'] = storm['mslp']
                                    storm['deferred_lambda0_location'] = storm['location']
                                    print(f"  → Storm {storm_id} near Hispaniola ({storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - DEFERRING λ₀ decision")
                                continue
                            
                            # If storm was deferred and has now moved out — accept the crossing
                            if storm.get('deferred_lambda0_step') is not None:
                                print(f"  → Storm {storm_id} moved away from Hispaniola to ({storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - ACCEPTING deferred λ₀")
                                # Fall through to the checks below using current location
                            ...
                            #     if abs(storm_lat - 18.4) < 0.5 and abs(storm_lon - (-70.0)) < 0.5:
                            #         z500 = y_with_mslp[0, 69, 0].cpu().numpy()
                            #         z500_nbhd = z500[i_min:i_max, j_min:j_max]
                            #         z500_center = z500[lat_idx, lon_idx]
                            #         if not np.all(z500_center <= z500_nbhd):
                            #             print(f"  → Storm {storm_id} known Hispaniola artifact cell (no Z500 min) {storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                            #             storm['saved'] = True
                            #             continue
                            #     # # Check 1: Low center not aligned -- smoothed field check — artifact vanishes when smoothed <-- this one is a weak filter
                            #     # if not np.all(center_val <= nbhd):
                            #     #     print(f"  → Storm {storm_id} Hispaniola terrain artifact {storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                            #     #     storm['saved'] = True
                            #     #     continue
                            #     # # Check 2: raw below λ₀ but smoothed above → terrain-induced spike <-- this one is a strong filter, maybe too strong
                            #     # if mslp_smooth[lat_idx, lon_idx] > lambda_0:
                            #     #     print(f"  → Storm {storm_id} Hispaniola terrain artifact (smoothed above λ₀) {storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                            #     #     storm['saved'] = True
                            #     #     continue
                            #     # Check 3: over land in this region <-- this one is a weak filter
                            #     # if self.land_sea_mask[lat_idx, lon_idx] > 0.5:
                            #     #     print(f"  → Storm {storm_id} Hispaniola land artifact {storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                            #     #     storm['saved'] = True
                            #     #     continue
                                
                            #     # Check 4: hard reject at known artifact grid cell (Dont use this)
                            #     # if abs(storm_lat - 18.4) < 0.5 and abs(storm_lon - (-70.0)) < 0.5:
                            #     #     print(f"  → Storm {storm_id} known Hispaniola artifact cell - REJECTING")
                            #     #     storm['saved'] = True
                            #     #     continue

                            # Reject storms over interior land (~100 km from coast)
                            if self.interior_land_mask[lat_idx, lon_idx]:
                                print(f"  → Storm {storm_id} over interior land ({storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                                storm['saved'] = True
                                continue

                            # Reject storms that drifted outside the basin
                            if not (self.basin['lat_min'] <= storm_lat <= self.basin['lat_max'] and
                                    self.basin['lon_min'] <= storm_lon <= self.basin['lon_max']):
                                print(f"  → Storm {storm_id} outside basin ({storm_lat:.1f}°N, {abs(storm_lon):.1f}°W) - REJECTING")
                                storm['saved'] = True
                                continue
                        ...
                        if storm.get('crossed_lambda0', False) and storm['mslp'] < self.state_B_threshold:
                            ...
                            if self.use_cps:
                                try:
                                    if pressure_interp is None:
                                        print("  → Computing pressure interp for B-state verification...")
                                        pressure_interp = self._compute_pressure_interp(y_phys, batch)
                                    ...
                                    with open(cps_log_path, 'a') as f:
                                        ...
                                        f.write(f"Time: {timestamp}\n")
                                        f.write(f"Location: ({storm['location'][0]:.1f}°N, {abs(storm['location'][1]):.1f}°W)\n")
                                        f.write(f"MSLP: {storm['mslp']:.1f} hPa\n")
                                        f.write(f"B: {cps['B']:.1f} m\n")
                                        f.write(f"-VT^L: {cps['VTL']:.1f} m\n")
                                        f.write(f"-VT^U: {cps['VTU']:.1f} m\n")
                                    ...
                                    if is_ET:
                                        print(f"  → Storm {storm_id} {cps['phase']} at B-state: NOT SAVED")
                                        del self.tracked_storms[storm_id]
                                        if storm_id in self.storm_track_history:
                                            del self.storm_track_history[storm_id]
                                        continue

                                    print(f"  → Storm {storm_id} TROPICAL at B-state: SAVED")

                                except Exception as e:
                                    print(f"  ⚠ CPS check failed for B-state storm {storm_id}: {e}")
                                    del self.tracked_storms[storm_id]
                                    if storm_id in self.storm_track_history:
                                        del self.storm_track_history[storm_id]
                                    continue
                            ...
                            crossing = self._create_crossing(
                                saved_states[step], step, storm['mslp'], -1,
                                batch, None, storm['location'], None, y_with_mslp,
                                cps_params=cps if self.use_cps else None
                            )
                            crossings.append(crossing)
                            ...
                            if self.use_cps and storm_id in self.storm_track_history:
                                del self.storm_track_history[storm_id]
                            print(f"  → Storm {storm_id} reached B: {storm['mslp']:.1f} hPa")
                            continue
    ...
    def generate_flux(self, initial_loader, n_trials):
        ...
        while True:
            ...
            # Count λ₀ and B crossings separately
            for crossing in result['crossings']:
                if crossing.interface_idx == 0:  # λ₀ crossing
                    worker_lambda0_crossings += 1
                elif crossing.interface_idx == -1:  # B event
                    worker_B_events += 1
        ...
        self.flux_estimate = worker_lambda0_crossings / total_days if total_days > 0 else 0
        self.direct_B_count = worker_B_events
        self.direct_B_rate = worker_B_events / total_days if total_days > 0 else 0
