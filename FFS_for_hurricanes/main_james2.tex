...
\begin{abstract}
We couple Forward Flux Sampling (FFS), a non-equilibrium rare-event technique from
statistical mechanics, to a fast neural weather emulator (SDL-WXFormer, 1$^\circ$
resolution) to estimate conditional tropical cyclogenesis rates without modifying model
dynamics. Tropical cyclone genesis probability varies by orders of magnitude across
synoptic regimes, yet direct ensemble sampling cannot resolve this variability at
operationally feasible ensemble sizes. FFS addresses this by decomposing the rare
$A \to B$ transition into a flux through an initial interface and a product of
conditional crossing probabilities across four shooting interfaces, exploiting the
natural parallelism of the shooting phase. The 1$^\circ$ emulator is chosen
deliberately for computational tractability: FFS requires $\mathcal{O}(10^4)$ model
trajectories per initial condition, and its calibrated stochastic layers provide the
ensemble spread the method requires.
...
capturing a clear seasonal cycle whose timing and amplitude are qualitatively consistent
with the known structure of the 2022 Atlantic season \citep{nhc2022season}
(suppressed early-season activity attributed to Saharan dust and wind shear,
followed by a highly active September).
A self-consistency check comparing FFS rates to independent direct-sampling rates from the
...
at the final intensification stages for the Ian environment.
The methodology is independent of model architecture and extends in principle to
...
into named systems \citep{Frank1970,gray1968global,Landsea1998}, but discriminating developing from
non-developing cases challenges deterministic and ensemble approaches alike
\citep{Titley2000,Halperin2013}. The challenge is not merely predictive: it is
statistical. For a genesis event with probability $p \sim 10^{-3}$ per day, direct
Monte Carlo estimation of the genesis rate to 20\% relative uncertainty requires
$\mathcal{O}(10^4)$ ensemble trajectories per initial condition, far beyond
operational or research budgets when using physics-based models. The question of
\textit{how frequently} genesis occurs under a given atmospheric state, not just
whether individual ensemble members develop, remains largely inaccessible to
conventional approaches.

Rare event sampling techniques address this by intelligently concentrating
computational effort along transition pathways.
The broader rare event literature includes transition path sampling
\citep{bolhuis2002transition} and metadynamics \citep{laio2002escaping}, which have
primarily been developed for molecular and condensed-matter systems.
The atmospheric community has primarily applied Adaptive Multilevel Splitting
\citep[AMS;][]{ragone2018computation,webber2019practical,galfi2019large} and related
trajectory resampling methods \citep{finkel2023revealing,finkel2024bringing}, which
efficiently generate ensembles of rare \textit{event realizations} and have been extended
to heat waves, cold spells, and circulation extremes, including direct application to
tropical cyclone intensification \citep{plotkin2019maximizing}.
A parallel and rapidly developing line of work couples these ideas
with machine learning emulators: \citet{lancelin2025ai} recently demonstrated
AI-boosted rare event sampling for characterizing extreme weather, establishing that
learned surrogates can dramatically reduce the cost of importance sampling in atmospheric
systems. Forward Flux Sampling \citep{allen2006forward,allen2009forward,valeriani2007computing,hussain2020studying}
occupies a distinct but complementary niche: rather than generating rare realizations,
FFS is designed specifically to estimate non-equilibrium steady-state \textit{transition
rates} (expected genesis events per unit time) with a factored uncertainty
structure that identifies \textit{which intensification stage is rate-limiting}.
For operational genesis prediction, this rate-centric view is the natural target quantity.

FFS requires no equilibrium assumptions: no free energy landscapes, detailed balance,
or Boltzmann distributions. The method needs only well-defined metastable regions,
measurable flux through interfaces, and Markovian dynamics \citep{allen2009forward}.
It has been applied across a broad range of non-equilibrium systems from
biochemical networks \citep{allen2005sampling} to nucleic acid folding kinetics
\citep{schreck2015dna}. The atmosphere is a canonical non-equilibrium steady-state
(NESS) system driven persistently by solar forcing and dissipated by radiation
and friction, and satisfies all three requirements. Atmospheric rare events are
dynamical transitions between regions of a strange attractor
\citep{lorenz1969atmospheric,touchette2009large}, not thermodynamic
transitions over energy barriers, making FFS \textit{more}
defensible here than equilibrium-based rare event techniques.
...
initial condition is a \textit{conditional genesis rate given the atmospheric state at
that IC}, not a climatological steady-state rate. Each FFS simulation treats a
10--15~day forecast window anchored to a specific ERA5 state as locally stationary,
so $k$ characterizes genesis probability density in that particular atmospheric
environment. This conditional framing is analogous to a conditional return period
given present climate state \citep{finkel2023revealing,finkel2024bringing}. It motivates
our seasonal sampling across 98~ICs and enables direct comparison against observed
Atlantic activity.

Machine learning weather models \citep{lam2023graphcast,bi2023pangu,pathak2022fourcastnet}
offer a natural platform for rare event sampling: they are $\sim$1000$\times$ faster
than physics-based models, natively generate ensemble spread through stochastic
parameterizations, and inherit the dynamical attractor of their training reanalysis.
This speed advantage is decisive for FFS, which requires $\mathcal{O}(10^4)$ trajectories
per initial condition. Recent work exploits this combination for
subseasonal-to-seasonal extreme event forecasting \citep{mu2025skillful} and importance
sampling of weather extremes \citep{lancelin2025ai,manshausen2025extreme}. SDL-WXFormer \citep{schreck2025sdl}
extends the standard deterministic emulator with stochastic decomposition
layers calibrated to ERA5 ensemble statistics, providing the trajectory diversity
the FFS shooting phase requires.
...
$\approx$0.24 GPU-minutes per single-member 15-day forecast on an NVIDIA A100, the full 98-IC dataset required
...
Whether the FFS rate calculation is \textit{physically} Markovian is a deeper question
that hinges on state-vector completeness: does the model's state vector capture all
variables that influence genesis probability?
SDL-WXFormer does not explicitly predict slow boundary-condition fields such as
sea-surface temperature (SST) or soil moisture.
These are held fixed at their initialisation values rather than evolving during the
simulation, though the model's weights encode multi-step climatological relationships
between these slow variables and atmospheric dynamics learned during training.
To the extent that boundary-condition fields at initialisation are correlated with
subsequent evolution encoded in the weights, this introduces a background-state
dependence that a fully Markovian FFS would not carry.
The self-consistency check described in Section~\ref{sec:results_val} can detect Markov
violations that produce systematic bias in the factorised rate relative to the direct
count, but cannot detect violations that bias both estimators in the same direction.
...
rates qualitatively consistent with the known structure of the 2022 Atlantic season; (2) a self-consistency
...
before returning to $A$ (Fig.~\ref{fig:schematic}). The order parameter $Q(\mathbf{x})$ is the minimum
...
MSLP is the community-standard metric for tropical cyclone intensity and is used
operationally by NHC, JTWC, and all major forecast centers to define genesis and
track intensity throughout a storm's lifecycle \citep{knaff2007improved}.
It has well-understood properties at 1$^\circ$ resolution (see state-$B$ threshold
discussion below), and is robustly trackable without requiring multi-variable composites
that would introduce additional calibration uncertainty.
We acknowledge that MSLP is a downstream integrator of the thermodynamic and kinematic
conditions that enable genesis, rather than a direct measure of those precursors.
However, for the purpose of defining the FFS interfaces and measuring crossing probabilities,
MSLP's role as a monotone descriptor of vortex organization is well suited: the seasonal
cycle resolved by the FFS rates (Figure~\ref{fig:rates}) is physically coherent, and the
geographic distribution of state-$B$ arrivals (Figure~\ref{fig:stateb_geo}) clusters in
the observed Atlantic main development region.
...
such extensions are expected to sharpen the physical interpretability of
per-interface bottleneck diagnostics but are not required to demonstrate that FFS
produces internally consistent rate estimates, which is the goal of this paper.
...
resolution through trial experiments (see Appendix~\ref{sec:si_threshold}).
...
test year 2022) at 1$^\circ$ resolution and 6-hour timesteps.
...
The screen was motivated primarily by MSLP artifacts arising from an imperfect terrain
file and 1$^\circ$ grid limitations, and secondarily by extratropical contamination
from recurving or baroclinic systems.
Both components were sufficient to demonstrate self-consistent FFS genesis rates in
this setting and should not be adopted uncritically for other grids or models.
...
and suppresses two known sources of spurious state-$B$ arrivals at 1$^\circ$ resolution:
...
grid noise at 1$^\circ$ resolution.
...
validation at this resolution and should be treated as engineering choices rather
...
Importantly, the IC date labels are \textit{initialisation} times, not genesis dates.
...
This structural agreement between two completely independent modelling systems
suggests that the intensification bottleneck identified by FFS reflects real atmospheric
physics rather than artefacts of the SDL-WXFormer architecture.
...
contamination screen rather than a rigorous warm-core classifier: it rejects obviously
...
sudden stratospheric warmings, blocking onset) and to physics-based models.
Any generative model producing an ensemble from a shared initial state (diffusion-based
emulator, flow-matching model, or stochastic parameterization scheme) satisfies the
architectural Markov requirement and can be coupled to FFS in the same way demonstrated here.
The combination is general: the same approach applies to any
low-probability, high-consequence atmospheric event where a rate estimate and
a bottleneck decomposition are more useful than a collection of rare realizations.
