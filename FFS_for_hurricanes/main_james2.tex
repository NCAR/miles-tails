...
It was motivated primarily by MSLP artifacts arising from an imperfect terrain file
and 1$^\circ$ grid limitations, and secondarily by extratropical contamination from
recurving or baroclinic systems.
...
grid-points) is applied to the MSLP field to locate local minima; the smoothed field
is used only to identify the grid-cell position of each minimum, and the raw
unsmoothed MSLP value at that cell is recorded as the order parameter.  Existing tracks are updated by nearest-neighbour matching within $12^\circ$;
...
consecutive timesteps of failed matching before being dropped.  All co-existing systems are tracked simultaneously; any storm that crosses $\lambda_0$
...
baroclinic cyclones entering the domain. The screen evaluates the thermal wind
parameters $-V_T^L$ and $-V_T^U$ at $\lambda_0$ crossings and when a tracked vortex
exceeds $50^\circ$N or $-10^\circ$W, rejecting cold-core systems with $-V_T^L < 0$
or $-V_T^U < 0$ (in practice, a numerical tolerance permits values down to $-100$~m
before a cold-core classification is assigned, accommodating grid noise at 1$^\circ$
resolution). The rejection thresholds were calibrated by manual inspection of a subset
of trajectories in the 2022 dataset that were visually identifiable as extratropical
(recurving systems and baroclinic cyclones intruding from the northwest); they are not
derived from formal CPS validation at this resolution and should be treated as
engineering choices rather than physical thresholds.
