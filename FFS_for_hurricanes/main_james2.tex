...
operationally feasible ensemble sizes; FFS resolves it by decomposing the rare
$A \to B$ transition into a series of tractable conditional crossing probabilities.
...
given present climate state \citep{finkel2023revealing,finkel2024bringing} and motivates
our seasonal sampling across 98~ICs and enables direct interpretation against observed
Atlantic activity.
...
per initial condition. Recent work has begun exploiting this combination for
subseasonal-to-seasonal extreme event forecasting \citep{mu2025skillful} and importance
sampling of weather extremes \citep{lancelin2025ai,manshausen2025extreme}. The SDL-WXFormer \citep{schreck2025sdl}
employed here extends the standard deterministic emulator with stochastic decomposition
...
The vortex tracker described below is a purpose-built component calibrated to the
SDL-WXFormer grid.
...
An empirically-calibrated contamination screen following the Cyclone Phase Space
...
hydrostatic extrapolation errors in the imperfect terrain file, and (2) mid-latitude
baroclinic cyclones entering the domain. The screen evaluates $-V_T^L$ and $-V_T^U$ at $\lambda_0$ crossings and when a
...
validate the Markov assumption (see below), nor does it guarantee that the absolute
rate values are unbiased with respect to the real atmosphere.

\textbf{Markovianity.}
FFS requires that a trajectory's future behaviour depends only on its current state,
not on its history prior to the most recent interface crossing.
For SDL-WXFormer, this question has two distinct dimensions.

\textit{Architectural Markovianity.}
Enabling rare-event sampling via FFS was an explicit design objective of SDL-WXFormer,
and the model was architected as a strict state-in/state-out emulator specifically
to satisfy the Markov requirement.
The base deterministic WXFormer is made stochastic by adding Stochastic Decomposition
Layer (SDL) modules and fine-tuning with a CRPS loss \citep{schreck2025sdl}.
At inference, each 6-hour forward step takes only the current atmospheric state vector
as input and applies the learned weights plus an independent stochastic perturbation
drawn from the SDL layers; no recurrent memory is carried between steps.
This design means the model is Markovian at inference by construction, to the extent
permitted by the available training data and the choice of state variables included.

\textit{Physical Markovianity and state-vector completeness.}
Whether the FFS rate calculation is physically Markovian is a separate question that
hinges on whether the model's state vector is sufficient to capture all variables that
influence genesis probability.
Variables absent from the state vector represent the primary source of implicit
non-Markovian pressure.
SDL-WXFormer does not explicitly predict slow boundary-condition fields such as
sea-surface temperature (SST) or soil moisture; these are held fixed at their
initialisation values rather than evolving during the simulation.
However, the model's weights encode multi-step climatological relationships between
these slow variables and atmospheric dynamics learned during training.
Trajectories launched from $\lambda_i$ over anomalously warm SST, or from
environments with anomalous soil moisture, will therefore evolve differently in ways
not captured by the atmospheric state vector alone.
To the extent that these boundary-condition fields at initialisation are correlated
with the subsequent evolution encoded in the weights, this introduces a background-state
...
probabilities) reflects \textit{cross-IC} atmospheric and SST variability, not
within-trajectory memory, and does not itself violate the Markov assumption.
The self-consistency check can detect Markov violations that produce systematic
bias in the factorized rate relative to the direct count, but cannot detect violations
that bias both estimators in the same direction.
We defer to future work a more rigorous test of whether $P(B|\lambda_i)$ depends
only on the current state or is also sensitive to SST and other omitted variables.
...
For the Earl IC specifically, Fig.~\ref{fig:reactive_traj} shows the 199 reactive
...
the system is likely to reach hurricane strength. For the Fiona IC, the uniformly elevated probabilities with a $P_4 = 0.276$ terminal
barrier indicate an environment that readily supports each successive intensification
step but where crossing the final 975~hPa threshold requires a specific convective
alignment not guaranteed even in this highly active environment.
For the Ian IC, the
...
the ensemble-mean reactive pathway, confirming that the pressure-based bottleneck
...
evolve progressively and consistently from $\lambda_0$ through state~$B$, validating
that the MSLP interfaces are tracking genuine tropical development rather than spurious
pressure minima.
...
Any generative weather or climate model that produces an ensemble of plausible
atmospheric trajectories from a shared initial state (whether a diffusion-based
emulator, a flow-matching model, or a stochastic parameterization scheme) satisfies
the architectural Markov requirement and can in principle be coupled to FFS in the same
way demonstrated here.
Neural emulators are fast enough to make FFS practical at scale, and any generative
model that produces an ensemble from a shared initial state satisfies the Markov
requirement. The combination is general: the same approach applies to any
