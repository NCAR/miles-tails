...
conditional crossing probabilities across four shooting interfaces. We use the
1$^\circ$ emulator because FFS requires $\mathcal{O}(10^4)$ model trajectories per
initial condition, and because the model's calibrated stochastic layers provide the
ensemble spread needed for the shooting phase.
...
The same calculation can be applied to higher-resolution physics-based models when
the trajectory cost is affordable.
\end{abstract}
...
Rare event sampling techniques address this by concentrating computation on the
transition itself. In atmospheric science, most work has used Adaptive Multilevel
Splitting \citep[AMS;][]{ragone2018computation,webber2019practical,galfi2019large}
and related trajectory-resampling methods
\citep{finkel2023revealing,finkel2024bringing}. Those methods are effective for
generating rare event realizations, including tropical cyclone intensification
\citep{plotkin2019maximizing}. Here the target is different. We want a transition
rate, not just examples of successful trajectories. FFS
\citep{allen2006forward,allen2009forward,valeriani2007computing,hussain2020studying}
is designed for that calculation. It also shows which stage of the transition is
hardest to cross, which is useful for genesis.
...
make this calculation practical because they are much faster than physics-based
models. That speed matters here because FFS requires $\mathcal{O}(10^4)$ trajectories
per initial condition. SDL-WXFormer \citep{schreck2025sdl} is useful for a second
reason: its stochastic decomposition layers produce the ensemble spread required by
the shooting phase.

\textbf{Scope and model choice.}
This paper asks a simple question: can FFS recover useful conditional genesis rates
from a fast weather emulator, and if so, what parts of genesis control those rates?
...
the intensities typical of major hurricane development. That limitation matters for
the threshold definition, but not for the main point here, which is whether the FFS
calculation is internally consistent and physically informative.
...
This can introduce background-state dependence that a fully Markovian FFS calculation
would not have.
