...
\graphicspath{
  ...
  {/glade/derecho/scratch/schreck/FFS/results_mar18/plots/review_figures/}
  {/glade/derecho/scratch/schreck/FFS/results_mar18/plots/physics_composites/}
}
\usepackage{natbib}
\usepackage{hyperref}
\usepackage{booktabs}
...
We demonstrate that Forward Flux Sampling (FFS), a non-equilibrium rare-event technique
from statistical mechanics, efficiently estimates genesis rates in a learned neural
weather emulator (SDL-WXFormer, 1$^\circ$ resolution) without modifying model dynamics.
...
capturing a clear seasonal cycle invisible to 50-member direct ensemble forecasting.
Internal validation --- comparing FFS rates to independent direct-sampling rates from the
same flux trajectories --- yields a mean ratio of $1.03 \pm 0.15$ across all 98 initial
conditions, confirming correct implementation of the conditional probability factorization.
...
Three case studies --- Hurricane Earl (September~2), a peak-activity environment
(September~9), and the Hurricane Ian precursor (September~22) --- reveal physically
distinct genesis bottlenecks: the rate-limiting step is initial tropical organization
for Earl, uniformly high crossing probabilities for the active-period environment, and
a hard barrier at the final intensification interface for the Ian precursor.
These contrasting probability structures encode distinct atmospheric physics and
demonstrate FFS as a diagnostic tool for mechanistic understanding of genesis pathways.
\end{abstract}
...
through interfaces, and Markovian dynamics \citep{allen2009forward}. The atmosphere is a
...
This paper demonstrates FFS for Atlantic tropical cyclogenesis in SDL-WXFormer,
applied to 98 initial conditions from the 2022 hurricane season. We report (1) a
three-order-of-magnitude seasonal cycle in genesis rates; (2) internal validation
showing direct-sampling and FFS rates agree within 3\% on average; (3) case studies
revealing physically distinct rate-limiting steps across atmospheric regimes; and
(4) a Cyclone Phase Space contamination screen ensuring sampled pathways correspond
to tropical rather than extratropical development.
...
State $B$: $Q \leq \lambda_B = 975$~hPa (tropical storm to minimal hurricane intensity).
The 975~hPa threshold is calibrated to the SDL-WXFormer's 1$^\circ$ horizontal
resolution: at this resolution the model reliably represents mesoscale circulation and
warm-core development through 975~hPa but cannot be expected to faithfully reproduce
intensity fluctuations at $\leq$960~hPa characteristic of major hurricanes, which
require convection-permitting resolution. The flux interface and four shooting
interfaces are:
\begin{equation*}
...
\subsection{Feature Tracking and CPS Contamination Screen}
...
\textbf{CPS contamination screen.}
A simplified Cyclone Phase Space \citep{hart2003cyclone} filter screens contamination
from two sources at 1$^\circ$ resolution: (1) spurious MSLP minima over high terrain
from hydrostatic extrapolation errors, and (2) mid-latitude baroclinic cyclones
entering the domain boundary. CPS verification is applied at $\lambda_0$ crossings
and when storms exceed $50^\circ$N or $-10^\circ$W, rejecting clearly extratropical
systems ($-V_T^L > 0$ or $-V_T^U > 0$). We note that formal CPS thresholds are not
validated for 1$^\circ$ model output; the filter is a practical contamination screen
rather than a rigorous warm-core classifier.
...
(consistent with the Fiona formation environment). This dynamic range is entirely
unresolvable by direct 50-member ensemble sampling: the ECMWF IFS 50-member
ensemble, analyzed over the same 98 initial conditions, reaches the minimum MSLP
threshold in 25.4\% of members on average. We note that this IFS statistic cannot
be directly interpreted as a genesis rate: IFS uses 0.25$^\circ$ physics coarsened
to 1.5$^\circ$ output, does not apply the decorrelated-crossing requirement that
defines $\lambda_0$ flux, and includes extratropical systems without a CPS filter.
We include this figure only as a contextual upper bound indicating that strong
MSLP deepening occurs at a frequency broadly consistent with SDL-WXFormer, while
acknowledging that day-to-day variability and the rate magnitude itself are
meaningful only for the FFS estimates.

The primary internal validation is the agreement between $k^{\text{FFS}}$ and the
independent direct-sampling rate $k^{\text{direct}}$ (Figure~\ref{fig:rates},
orange triangles; scatter comparison in Figure~\ref{fig:rate_scatter}). These estimates
are derived from entirely separate components of the calculation: $k^{\text{FFS}}$ is a
product of flux and shooting probabilities, while $k^{\text{direct}}$ counts raw state-$B$
arrivals during flux generation.
Across all 98 initial conditions, the ratio $k^{\text{FFS}}/k^{\text{direct}} = 1.03 \pm 0.15$
(mean $\pm$ std; individual ratios range 0.71--1.52). Agreement within 3\% on average
provides strong evidence that the conditional probability factorization is correctly
implemented and that the interface ensemble sizes are adequate.

Computational enhancement factors range from $3\times$ (most active IC) to $140\times$
(most suppressed), with geometric mean $14\times$ and arithmetic mean $26\times$;
the right-skewed distribution (geometric $\ll$ arithmetic mean) indicates a log-normal
speedup distribution, consistent with the FFS rate itself spanning orders of magnitude.
The geometric mean is the more appropriate summary statistic for log-normal quantities.
...
\midrule
All-98 mean$\pm$SD  & $0.065\pm0.034$ & $0.278\pm0.154$ & $0.674\pm0.086$ & $0.569\pm0.107$ & $0.356\pm0.093$ & $2.6\times10^{-3}$ & $2.5\times10^{-3}$ & $1.03\pm0.15$ & $26\pm27\times$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
color-coded by branching factor (cluster weight); the 201 reactive pathways sample
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.32\textwidth]{ffs_tree_rank05_2022-09-02T00Z_lambda0_config_0175_JR.png}\hfill
\includegraphics[width=0.32\textwidth]{ffs_tree_rank04_2022-09-09T12Z_lambda0_config_0545_MZ.png}\hfill
\includegraphics[width=0.32\textwidth]{ffs_tree_rank02_2022-09-22T00Z_lambda0_config_0917_OV.png}
...
(Table~\ref{tab:results}). The FFS and IFS curves agree in overall shape, with the FFS
predicting systematically higher committor values at each interface, reflecting the
higher genesis rate of the AI model relative to IFS at 0.25$^\circ$/1.5$^\circ$.
...
Figure~\ref{fig:composites} shows storm-centred composites of vorticity, relative humidity,
and warm-core anomaly, composited across all reactive pathways for each case-study IC.
...
The primary difference among the ICs appears in the earlier interfaces: the Earl IC
($k = 1.8\times10^{-3}$~day$^{-1}$) shows weaker vorticity organisation at $\lambda_1$
compared to the active-period IC, consistent with the lower $P_1$ value for that case
(Table~\ref{tab:results}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{all_ics_composites.png}
\caption{Storm-centred physics composites along reactive pathways, averaged over all
reactive configurations for the three case-study ICs combined (all~ICs panel).
Columns show interfaces $\lambda_0$ through state~$B$; sample sizes $n$ are given
in each column header.
\textit{Row 1}: 850-hPa relative vorticity $\zeta_{850}$ ($10^{-5}$~s$^{-1}$).
\textit{Row 2}: 700-hPa relative humidity RH$_{700}$ (\%).
\textit{Row 3}: 500-hPa warm-core anomaly $T'_{500}$ (K, relative to 0--30$^\circ$N mean).
The progressive intensification of the vortex core and warm anomaly from $\lambda_0$
to state~$B$ is robust across ICs, confirming that the FFS reactive pathways capture
physically coherent tropical cyclogenesis rather than model noise.}
...
Enhancement factors (3--140$\times$, geometric mean 14$\times$, arithmetic mean 26$\times$)
scale inversely with genesis probability, as expected from FFS theory \citep{allen2009forward}.
The log-normal character of the speedup distribution (geometric mean substantially below
arithmetic mean) mirrors the log-normal FFS rate distribution itself and motivates
reporting geometric rather than arithmetic means for speedup summaries. For the most active
environments where direct sampling is feasible (speedup $3\times$), the two methods
serve as mutual validation. For suppressed environments (speedup $\sim$100$\times$),
...
% AGU requires a data availability statement for all manuscripts.
% Suggested text (update DOIs/repositories before submission):
%   The FFS simulation output (rate statistics, conditional probabilities, and
%   reactive pathway archives for all 98 initial conditions) will be archived at
%   [repository, e.g., Zenodo or NCAR RDA] upon acceptance.
%   The SDL-WXFormer model weights and inference code are available at
%   \url{https://github.com/[repo]}.
%   ERA5 reanalysis data are available from the Copernicus Climate Data Store
%   (\url{https://cds.climate.copernicus.eu}).
\textbf{[Data availability statement required before submission. See comment above.]}
