...
resolution through trial experiments. Initial runs used $\lambda_B = 965$~hPa, but at
this threshold state-$B$ arrivals were disproportionately found at high latitudes
($>40^\circ$N) and frequently misclassified by the CPS filter due to the model's
limited representation of intense warm-core structure at fine pressure values.
Raising the threshold to 975~hPa shifted state-$B$ arrivals to physically
expected geographic regions and reduced extratropical contamination substantially
(see Supplementary Information for sensitivity analysis).
At 975~hPa the model reliably represents mesoscale circulation and warm-core development,
corresponding to tropical storm to minimal hurricane intensity.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.58\textwidth]{2022-09-02T00Z_composites.png}\\[1pt]
\includegraphics[width=0.58\textwidth]{2022-09-09T12Z_composites.png}\\[1pt]
\includegraphics[width=0.58\textwidth]{2022-09-22T00Z_composites.png}
\caption{Storm-centred physics composites along reactive pathways for each case-study IC.
\textit{Top}: Earl IC (2022-09-02T00Z).
\textit{Middle}: Fiona precursor IC (2022-09-09T12Z).
\textit{Bottom}: Ian precursor IC (2022-09-22T00Z).
Within each panel, columns show interfaces $\lambda_0$ through state~$B$; sample sizes
...
The Earl composites show weaker vorticity and warm-core development at $\lambda_1$
relative to the Fiona and Ian composites, consistent with Earl's lower $P_1$ value
and the identification of initial organisation as the rate-limiting bottleneck for
that environment.}
