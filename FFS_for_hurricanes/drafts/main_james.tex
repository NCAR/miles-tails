...
with the known structure of the 2022 Atlantic season (suppressed early-season activity
attributed to Saharan dust and wind shear, followed by a highly active September).
...
computational effort along transition pathways. The atmospheric community has
primarily applied Adaptive Multilevel Splitting \citep[AMS;][]{ragone2018computation,
webber2019practical} and related trajectory resampling methods
\citep{finkel2023revealing,finkel2024bringing}, which efficiently generate ensembles of
rare \textit{event realizations} and have been extended to heat waves, cold spells, and
circulation extremes. A parallel and rapidly developing line of work couples these ideas
...
systems. Forward Flux Sampling \citep{allen2006forward,allen2009forward,hussain2020studying}
...
dynamical transitions between regions of a strange attractor, not thermodynamic
transitions over energy barriers \citep{touchette2009large}, making FFS \textit{more}
...
track intensity throughout a storm's lifecycle.
...
We test this implicitly through the self-consistency check: if significant
...
memory, and does not violate Markovianity.
It is important to be precise about the limits of this implicit test.
The self-consistency check can detect Markov violations that produce
\emph{systematic} bias in the factorized rate relative to the direct count;
for example, if conditioning on a particular interface level consistently selects
trajectories with anomalous histories that cause the product $\prod P_i$ to deviate
from the unconditional rate.
What the self-consistency check cannot detect is a Markov violation that biases
$k^{\text{FFS}}$ and $k^{\text{direct}}$ \emph{in the same direction}: if the SDL-WXFormer's
stochastic layers produce trajectories with correlated memory across interfaces
(e.g., a persistent large-scale environment that simultaneously makes all $P_i$
easier to cross), both estimates would be inflated together and their ratio would
remain near unity while the absolute rate is wrong.
Such a scenario would require an explicit committor analysis of whether
$P(B|\lambda_i)$ is a function of $Q$ alone, independent of trajectory history,
which is beyond the scope of the present methods demonstration.
The physical plausibility of approximate Markovianity in this setting rests on the
observation that the SDL-WXFormer's stochastic layers generate independent noise
at each 6-hour timestep, so the primary source of memory is the resolved atmospheric
state itself (which is captured in the model's state vector).
A more rigorous test comparing conditional probabilities for trajectories with
different histories at the same interface is deferred to future work but is not
...
The Earl tree (2022-09-02T00Z, 18~hours before Earl's genesis) seeds near 18$^\circ$N
...
The central self-consistency result is that FFS and direct-sampling rates agree with a mean
ratio of $1.03\pm0.15$ across 98 initial conditions, confirming
the method's correct implementation. These are entirely independent estimates derived
from the same simulation data: the FFS rate uses only the interface crossing counts
...
the system is likely to reach hurricane strength. For the Ian IC, the
