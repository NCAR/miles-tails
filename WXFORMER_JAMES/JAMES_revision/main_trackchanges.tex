...
%DIF DEL /tmp/main_submitted.tex   Mon Jul 27 15:21:07 2026
%DIF ADD main.tex                  Mon Jul 27 15:20:59 2026
\documentclass{agujournal2019}
...
, so inference step count alone no longer separates these methods from single-pass alternatives; the remaining distinction is whether a separate generative model and distillation stage are required at all}\DIFaddend .
...
\DIFaddbegin \subsection{Multivariate and Spatial-Dependence Diagnostics}
\label{sec:multivariate}

\DIFadd{The diagnostics above are univariate. To test whether the learned perturbations capture spatial and cross-variable dependence, we add two multivariate measures. First, the variogram score of order $p=0.5$ \mbox{%DIFAUXCMD
\cite{scheuerer2015variogram}}\hskip0pt%DIFAUXCMD
, which is specifically sensitive to spatial dependence structure, computed for the SDL ensemble over twelve 2022 initializations (the first of each month, 00Z) at 24, 120, and 240\,h for Z500, T850, and Q850 on a spatially pooled grid (every sixth point; pairwise sums scale quadratically in the number of points). The score grows monotonically with lead time for every field, with the largest relative growth for Z500 (a factor of $\sim$49 from 24\,h to 240\,h, versus $\sim$13 for T850 and $\sim$5 for Q850), consistent with Z500's longer error-growth timescale. Our archives hold IFS-ENS only as summary statistics and regridded distribution products rather than raw member fields at matching valid times, so we report the SDL variogram score as a documented baseline with its lead-time scaling rather than as a head-to-head comparison.
}

\DIFadd{Second, and more directly interpretable, the cross-variable spatial correlation between Z500 and T850, computed per member and compared against the same correlation in ERA5 at each of the 36 initialization-lead combinations. The SDL members track the ERA5 coupling essentially exactly: mean member correlation 0.939 versus 0.936 in ERA5, mean absolute difference 0.004, with per-combination differences within $[-0.024, +0.016]$ and no systematic sign bias, out to 240\,h. Thus the stochastic perturbations do not degrade this physically meaningful cross-variable relationship even at 10 days.
}
...
\DIFaddbegin \DIFadd{A complementary way to isolate each injection point's contribution is single-scale elimination: setting one $\beta_\ell = 0$ removes that site's perturbation exactly while fresh independent noise is drawn at the remaining sites. Table~\ref{tab:elimination} reports 20-member ensembles for $\beta = [0,1,1]$, $[1,0,1]$, and $[1,1,0]$ against the $[1,1,1]$ control on the same Winter Storm Izzy case. The result is specific: eliminating the intermediate (Level~2) injection point collapses ensemble spread far more than eliminating either of the other two. At 108\,h its SSR falls to roughly half the control value in every field examined, and it produces the worst RMSE and CRPS of the three eliminations in every field at every lead time tested, whereas eliminating the coarsest or finest site leaves both spread and skill within roughly 10\% of the control. This is consistent with the Level-2 site's broad wavenumber footprint in Figure~\ref{fig:ke_spectra}c. Note that this is an inference-time elimination on a single case, not a retrained ablation; the two answer related but distinct questions, since a model fine-tuned without a given site could partially compensate through the remaining ones.
}
...
\midrule
\DIFaddFL{MSLP (Pa) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{255.6 }& \DIFaddFL{228.4 }& \DIFaddFL{0.89 }& \DIFaddFL{178.0 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{250.4 }& \DIFaddFL{207.8 }& \DIFaddFL{0.83 }& \DIFaddFL{175.0 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{281.0 }& \DIFaddFL{147.0 }& \DIFaddFL{0.52 }& \DIFaddFL{223.3 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{249.8 }& \DIFaddFL{226.7 }& \DIFaddFL{0.91 }& \DIFaddFL{172.0 }\\
\midrule
\DIFaddFL{Z500 (m) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{490.2 }& \DIFaddFL{216.1 }& \DIFaddFL{0.44 }& \DIFaddFL{438.7 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{484.9 }& \DIFaddFL{200.8 }& \DIFaddFL{0.41 }& \DIFaddFL{435.3 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{494.4 }& \DIFaddFL{143.4 }& \DIFaddFL{0.29 }& \DIFaddFL{456.7 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{485.6 }& \DIFaddFL{215.4 }& \DIFaddFL{0.44 }& \DIFaddFL{434.5 }\\
\midrule
\DIFaddFL{T850 (K) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{1.260 }& \DIFaddFL{0.973 }& \DIFaddFL{0.77 }& \DIFaddFL{0.921 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{1.239 }& \DIFaddFL{0.906 }& \DIFaddFL{0.73 }& \DIFaddFL{0.906 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{1.344 }& \DIFaddFL{0.673 }& \DIFaddFL{0.50 }& \DIFaddFL{1.085 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{1.234 }& \DIFaddFL{0.962 }& \DIFaddFL{0.78 }& \DIFaddFL{0.888 }\\
\bottomrule
\end{tabular}
\caption{\DIFaddFL{Single-scale elimination at T+108\,h for the Winter Storm Izzy case (2022-01-13 00Z initialization): ensemble-mean RMSE, ensemble spread, spread-skill ratio, and CRPS from 20-member ensembles in which one injection point's gain is set to zero at inference while independent noise is drawn at the remaining sites. Eliminating the Level-2 (meso-$\alpha$) site roughly halves SSR and degrades RMSE and CRPS in every field, while eliminating Level~1 or Level~3 leaves spread and skill within roughly 10\% of the control. Inference-time elimination on a single case, not a retrained ablation.}}
