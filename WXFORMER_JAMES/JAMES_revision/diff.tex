...
%DIF DEL ../JAMES/main.tex   Tue Jul 28 11:34:17 2026
%DIF ADD main.tex            Thu Aug 13 11:49:23 2026
\documentclass{agujournal2019}
...
\item \DIFdelbegin \DIFdel{Compact latent storage enables perfect forecast reproducibility and post-inference spread adjustment without regenerating members
}\DIFdelend \DIFaddbegin \DIFadd{Ensemble members are archived as kilobyte-scale latent vectors, enabling exact regeneration and spread adjustment by re-running only the decoder
}\DIFaddend \item Multi-scale perturbation structure enables operational control over uncertainty: \DIFdelbegin \DIFdel{post-inference }\DIFdelend \DIFaddbegin \DIFadd{post-hoc }\DIFaddend spread adjustment, ensemble member interpolation, and scale-targeted perturbations
\end{itemize}

\section*{Abstract}
AI weather prediction ensembles with latent noise injection and optimized with the continuous ranked probability score (CRPS) have produced both accurate and well-calibrated predictions with far less computational cost compared with diffusion-based methods. However, current CRPS ensemble approaches vary in their training strategies and noise injection mechanisms, with most injecting noise globally throughout the network via conditional normalization. \DIFdelbegin \DIFdel{This structure increases training expense and limits the physical interpretability of the stochastic perturbations }\DIFdelend \DIFaddbegin \DIFadd{Because the stochasticity enters through a single global pathway, the resulting perturbations are not separately addressable by spatial scale after inference}\DIFaddend . We introduce Stochastic Decomposition Layers (SDL) for converting deterministic machine learning weather models into probabilistic ensemble systems. Adapted from StyleGAN's hierarchical noise injection, SDL applies learned perturbations at three decoder scales through latent-driven modulation, per-pixel noise, and channel scaling. When applied to WXFormer via transfer learning, SDL requires less than 2\% of the computational cost needed to train the baseline model. Each ensemble member is \DIFdelbegin \DIFdel{generated from a compact latent tensor (5 MB}\DIFdelend \DIFaddbegin \DIFadd{determined by compact per-scale latent vectors (5.3 kB per member per forecast step, together with the noise-generator seed}\DIFaddend ), enabling \DIFdelbegin \DIFdel{perfect reproducibility }\DIFdelend \DIFaddbegin \DIFadd{exact regeneration }\DIFaddend and post-inference spread adjustment through \DIFdelbegin \DIFdel{latent rescaling }\DIFdelend \DIFaddbegin \DIFadd{rescaling of the stored perturbations, at the cost of re-running only the decoder (23\% of a full forward pass)}\DIFaddend . Evaluation on 2022 ERA5 reanalysis shows ensembles with spread-skill ratios approaching unity and rank histograms that progressively flatten toward uniformity through medium-range forecasts, achieving calibration competitive with operational IFS-ENS. Multi-scale experiments reveal hierarchical uncertainty: coarse layers modulate synoptic patterns while fine layers control mesoscale variability. The explicit latent parameterization provides \DIFdelbegin \DIFdel{interpretable uncertainty quantification }\DIFdelend \DIFaddbegin \DIFadd{a scale-interpretable representation of forecast uncertainty }\DIFaddend for operational forecasting and climate applications.

\section*{Plain Language Summary}
Ensemble forecasting quantifies weather uncertainty by generating multiple plausible scenarios, but traditional methods require expensive repeated simulations. We developed a method that converts single scenario machine learning models into ensemble systems efficiently. Our approach learns uncertainty structure from historical data and stores each \DIFdelbegin \DIFdel{forecast as a small file (5 MB}\DIFdelend \DIFaddbegin \DIFadd{ensemble member as a set of small files (kilobytes per forecast step}\DIFaddend ), enabling exact reproduction and \DIFdelbegin \DIFdel{post-inference adjustment without regenerating forecasts}\DIFdelend \DIFaddbegin \DIFadd{inexpensive adjustment of the spread after a forecast has been made}\DIFaddend . Testing shows calibrated ensembles competitive with operational systems while requiring less than 2\% of baseline training cost. The method provides forecasters with systematic uncertainty exploration capabilities for operational prediction.
...
In this work, we introduce Stochastic Decomposition Layers (SDL)\DIFdelbegin \DIFdel{that separate latent-driven modulation from }\DIFdelend \DIFaddbegin \DIFadd{, a mechanism for adding controlled randomness to a deterministic forecast model. An SDL injects a random perturbation into the model's internal feature maps at several stages of the decoder, each stage operating at a different spatial resolution, so that each injection point influences a different range of atmospheric scales. The perturbation at each point combines a low-dimensional random vector, which sets the overall character of that member's perturbation, with independent }\DIFaddend per-pixel \DIFdelbegin \DIFdel{stochasticity, enabling }\DIFdelend \DIFaddbegin \DIFadd{noise that supplies local detail. Both are shaped by weights learned during fine-tuning. This separation of scale-setting structure from local stochasticity is what enables }\DIFaddend controlled exploration of forecast uncertainty\DIFaddbegin \DIFadd{, and the formal definition is given in Section~2.2}\DIFaddend . We build upon the WXFormer architecture \cite{schreck2025, chapman2025camulator}, incorporating SDL at multiple hierarchical scales to enable two critical capabilities: (1) calibrated probabilistic forecasts competitive with physics-based ensembles, achieved through CRPS-based training with computational efficiency comparable to other computationally efficient methods, and (2) post-inference exploration of forecast uncertainty by systematically perturbing latent tensor and noise components. While noise scaling is theoretically possible in any stochastic model, SDL learns a symmetric latent geometry that provides calibrated control over ensemble spread\DIFdelbegin \DIFdel{without regeneration}\DIFdelend . Each ensemble member is determined by a compact \DIFdelbegin \DIFdel{latent tensor, enabling perfect }\DIFdelend \DIFaddbegin \DIFadd{stored representation, enabling exact }\DIFaddend reproducibility and spread adjustment after inference \DIFaddbegin \DIFadd{at the cost of re-running only the decoder (Section~2.3)}\DIFaddend . This provides operational control over forecast uncertainty without \DIFdelbegin \DIFdel{regenerating ensemble members}\DIFdelend \DIFaddbegin \DIFadd{archiving or regenerating the full ensemble fields}\DIFaddend .
...
\DIFdelbeginFL \DIFdelFL{$\alpha$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\sigma_0$ }\DIFaddendFL & fixed noise scaling & scalar \\
output & \DIFdelbeginFL \DIFdelFL{$\mathbf{F}_{\text{in}} + \alpha \cdot \mathbf{R} \odot \mathbf{S} \odot \mathbf{M}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathbf{F}_{\text{in}} + \sigma_0 \cdot \mathbf{R} \odot \mathbf{S} \odot \mathbf{M}$ }\DIFaddendFL & $(B, C, H, W)$ \\
\bottomrule
\end{tabular}
...
\begin{equation}
\mathbf{F}_{\text{out}} = \mathbf{F}_{\text{in}} + \DIFdelbegin \DIFdel{\alpha }\DIFdelend \DIFaddbegin \DIFadd{\sigma_0 }\DIFaddend \cdot \mathbf{R}_{\text{Gauss}}  \odot \mathbf{S} \odot \mathbf{M}
\label{eq:sdl}
\end{equation}
where $\mathbf{R}_{\text{Gauss}} \sim \mathcal{N}(0, \mathbf{I})$ is per-pixel Gaussian noise sampled at full spatial resolution, \DIFdelbegin \DIFdel{$\alpha$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_0$ }\DIFaddend is a non-trainable scaling parameter, $\mathbf{S} \in \mathbb{R}^{B \times C \times 1 \times 1}$ is a tensor derived from a latent tensor $\mathbf{Z}_\ell \sim \mathcal{N}(0, \mathbf{I})$ sampled independently at each SDL layer via learned \DIFdelbegin \DIFdel{linear }\DIFdelend \DIFaddbegin \DIFadd{affine }\DIFaddend transformation, and $\mathbf{M} \in \mathbb{R}^{1 \times C \times 1 \times 1}$ is a learnable channel-wise modulation parameter. The operator $\odot$ denotes element-wise multiplication with broadcasting across spatial dimensions. \DIFaddbegin \DIFadd{Note that the perturbation is bilinear in the two noise sources. It is a product of the independent Gaussians $\mathbf{R}_{\text{Gauss}}$ and (an affine function of) $\mathbf{Z}_\ell$, and is therefore symmetric but heavy-tailed, i.e., already non-Gaussian before the downstream decoder nonlinearities act on it. }\DIFaddend Table~\ref{tab:tensor_shapes} details the tensor operations and shape transformations implementing this layer. Unlike StyleGAN's original implementation which uses correlated noise across hierarchical levels, we sample independent latent vectors $\mathbf{Z}_\ell \sim \mathcal{N}(0, \mathbf{I})$ at each SDL injection point to allow distinct uncertainty representations at different scales, where synoptic/meso-$\alpha$ (400-800~km) and meso-$\beta$ (100-200~km) processes have different predictability characteristics. The multiplicative structure in Equation~\ref{eq:sdl} enables independent control over ensemble characteristics: the linear transformation producing $\mathbf{S}$ and the channel modulation parameter $\mathbf{M}$ are learned during training, while $\alpha$ remains fixed to provide consistent noise scaling. During inference, sampling different $\mathbf{Z}$ tensors produces distinct ensemble members, with the learned parameters ensuring perturbations remain appropriately scaled and physically plausible.
...
To enable extensive experimentation within computational constraints, we implement decoder-only SDL injection with independent latent sampling at each layer. \DIFaddbegin \DIFadd{Retrained ablations of both choices are reported in Section~5. }\DIFaddend We refer to architectural positions as ``decoder levels'' (Level~1 through Level~4, Table~\ref{tab:decoder_scales}) and SDL application locations as ``injection points'' controlled by parameters $\beta_1$, $\beta_2$, and $\beta_3$ respectively, spanning from the synoptic/meso-$\alpha$ boundary (Level~1) through the meso-$\beta$ scale (Level~4). \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The decoder-only configuration requires storing three latent tensors per ensemble member (one per injection point), totaling approximately 5 MB }\DIFdelend \DIFaddbegin \DIFadd{The parameters $\beta_1$, $\beta_2$, and $\beta_3$ are per-scale gains applied at inference time to the stochastic perturbation injected at each point. They are fixed at unity throughout training and varied only afterwards, which is what allows ensemble spread to be adjusted by spatial scale on an already-generated forecast (Section~\ref{sec:multi_scale_control}).
}

\DIFadd{Because an ensemble member is fully determined by what enters the three injection points at each forecast step, there are two legitimate archival options, and which is preferable depends on the use case. Option~A stores the raw latent vectors together with the seed of the random-number generator: three 442-dimensional latents per member per step, $3 \times 442 \times 4\,\mathrm{B} = 5.3$~kB }\DIFaddend in single precision\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, roughly $3\times10^{3}$ times smaller than the corresponding forecast fields ($71 \times 192 \times 288$ }\DIFaddend float32 \DIFdelbegin \DIFdel{): Level 1 (24$\times$36$\times$71, $\sim$240 KB), Level 2 (48$\times$72$\times$71, $\sim$960 KB), and Level 3 (96$\times$144$\times$71, $\sim$3.7 MB ). Full encoder-decoder injection would require six tensors ($\sim$10 MB per member ). These compact latent representations enable perfect }\DIFdelend \DIFaddbegin \DIFadd{$= 15.7$~MB per member per step). The latents alone do not determine the member, because the per-pixel noise $\mathbf{R}_{\text{Gauss}}$ is drawn inside each layer. Exact regeneration under Option~A therefore requires the stored seed as well (a few integers, negligible in size, but part of the recipe). Option~B stores the realized per-layer perturbation tensors after $\mathbf{R}_{\text{Gauss}}$ is drawn, which removes the seed dependence entirely and makes replay self-contained, at a cost of 49.5~MB per member per step ($1024{\times}36{\times}48 + 512{\times}72{\times}96 + 256{\times}144{\times}192$ float32 values on the internally padded grid). This is larger than the forecast fields themselves, so Option~B trades storage for self-containment rather than saving space. Either representation enables exact }\DIFaddend reproducibility of any ensemble member and \DIFdelbegin \DIFdel{support }\DIFdelend \DIFaddbegin \DIFadd{supports }\DIFaddend post-inference spread adjustment through \DIFdelbegin \DIFdel{latent rescaling , all while requiring orders of magnitude less storage than archiving complete forecast fields}\DIFdelend \DIFaddbegin \DIFadd{rescaling of the stored perturbations, at the cost of re-running only the decoder (Section~\ref{sec:reproducibility})}\DIFaddend .

Each injection point employs a fixed scaling parameter \DIFdelbegin \DIFdel{$\alpha$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_0$ }\DIFaddend to accommodate feature magnitude variations across network depth. \DIFdelbegin \DIFdel{Appropriate $\alpha$ selection enables reasonable }\DIFdelend \DIFaddbegin \DIFadd{We set $\sigma_0 = 0.235$ at all injection points, selected by a short sweep over candidate values as the setting that minimized the training loss at initialization. It was not tuned against validation skill. A one-epoch sweep over $\sigma_0 \in \{0.10, 0.235, 0.50\}$ confirms the criterion. The value $\sigma_0 = 0.235$ attains the lowest training and validation loss after one epoch, roughly 7--8\% below either alternative (Supporting Information Figure~S3). The reason $\sigma_0$ is held fixed rather than learned is that the perturbation in Equation~\ref{eq:sdl} is linear in $\sigma_0$ and in both learned components, so any rescaling of $\sigma_0$ is absorbed exactly by $\mathbf{S}$ or $\mathbf{M}$. Thus $\sigma_0$ is redundant with parameters the model already learns, and its role is confined to setting the initialization scale, where an appropriate value gives the }\DIFaddend initial ensemble spread \DIFdelbegin \DIFdel{, accelerating convergence of }\DIFdelend \DIFaddbegin \DIFadd{a sensible magnitude relative to the deterministic 6-hour error and accelerates convergence of the }\DIFaddend learnable SDL components\DIFdelbegin \DIFdel{($\mathbf{M}$ and the linear transformation generating $\mathbf{S}$)}\DIFdelend . The transfer learning framework allows freezing pre-trained WXFormer weights while training only SDL parameters, or joint fine-tuning \DIFdelbegin \DIFdel{for superior ensemble quality. We set $\alpha = 0.235$ at all injection points based on preliminary experiments. }\DIFdelend \DIFaddbegin \DIFadd{of all weights. We use the latter in this work, a choice tested directly by a retrained frozen-backbone ablation in Section~5.
}\DIFaddend 

\subsection{Ensemble Generation}
During inference, multiple ensemble members are generated by sampling different latent vectors $\{\mathbf{Z}_1, \mathbf{Z}_2, \mathbf{Z}_3\}$ independently at each of the three decoder SDL injection points. The learned parameters $\mathbf{M}$ and the linear transformations producing $\mathbf{S}$, combined with the fixed \DIFdelbegin \DIFdel{$\alpha$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_0$ }\DIFaddend scaling parameters, ensure variations remain physically plausible and appropriately scaled. \DIFaddbegin \DIFadd{Noise is re-injected at every autoregressive step. At each 6\,h step $t$ of the rollout, an independent latent $\mathbf{Z}_\ell^{(t)} \sim \mathcal{N}(0,\mathbf{I})$ is drawn at each of the three injection points $\ell$, so that an $N$-step forecast consumes $3N$ latent draws per member. Noise is not held fixed along the trajectory, nor applied only at initialization. A member is thus identified by the complete ordered set $\{\mathbf{Z}_\ell^{(t)}\}$, and it is this set that is archived (Section~5.4). }\DIFaddend This multi-scale injection produces comprehensive uncertainty quantification: deep-layer perturbations affect large-scale flow patterns (e.g., synoptic troughs and ridges), while shallow-layer perturbations modulate fine-scale features (e.g., mesoscale convective systems and surface temperature gradients). The ensemble spread reflects the model's learned uncertainty structure rather than relying on ad-hoc perturbation strategies.

\section{Training dataset}
This study uses \DIFaddbegin \DIFadd{global, 6-hourly }\DIFaddend ERA5 reanalysis \cite{hersbach2020era5} \DIFdelbegin \DIFdel{coarsened to 1}\DIFdelend \DIFaddbegin \DIFadd{spanning 1979-2022, regridded from the native 0.25}\DIFaddend $^\circ$ \DIFdelbegin \DIFdel{horizontal resolution (360}\DIFdelend \DIFaddbegin \DIFadd{archive using conservative interpolation (following the approach of WeatherBench2, see \mbox{%DIFAUXCMD
\cite{sha2025conservation, sha2025terrain}}\hskip0pt%DIFAUXCMD
) to a 192}\DIFaddend $\times$\DIFdelbegin \DIFdel{181 grid ) spanning 1979-2022. }\DIFdelend \DIFaddbegin \DIFadd{288 grid (approximately 0.94$^\circ$ latitude $\times$ 1.25$^\circ$ longitude, the CESM finite-volume grid). This single grid is used end-to-end for training, fine-tuning, inference, and verification (Appendix B1). }\DIFaddend We selected 16 vertical levels from ERA5's 137 model levels (levels 10, 30, 40, 50, 60, 70, 80, 90, 95, 100, 105, 110, 120, 130, 136, 137), along with surface variables, static fields (land-sea mask, surface geopotential), and instantaneous solar irradiance as forcing, yielding 74 total input variables. This vertical discretization emphasizes lower-to-mid tropospheric representation where most weather-relevant processes occur while accepting coarser resolution at higher altitudes. The temporal split follows \cite{schreck2025}: training (1979-2017), validation (2018-2019), and testing (2020-2022)\DIFaddbegin \DIFadd{, of which the 2022 00Z initializations are used for all verification reported here}\DIFaddend . Complete details on variable selection, vertical level specification, and data processing are provided in \cite{schreck2025, sha2025terrain}.
...
We initialize from a pre-trained deterministic WXFormer model trained on ERA5 following \cite{schreck2025}. The SDL components (\DIFdelbegin \DIFdel{$\alpha$}\DIFdelend \DIFaddbegin \DIFadd{$\sigma_0$}\DIFaddend , $\mathbf{M}$, and the linear transformations producing $\mathbf{S}$) are inserted into the model, and all model weights are then fine-tuned jointly. This transfer learning approach \DIFdelbegin \DIFdel{leverages }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend the pre-trained model's deterministic forecast skill as a strong initialization\DIFdelbegin \DIFdel{, requiring }\DIFdelend \DIFaddbegin \DIFadd{. The fine-tuning increment is }\DIFaddend approximately 1.61\% of the computational cost \DIFdelbegin \DIFdel{needed to train }\DIFdelend \DIFaddbegin \DIFadd{of training }\DIFaddend the baseline WXFormer from scratch (see \DIFdelbegin \DIFdel{ref }\DIFdelend \cite{schreck2025} for baseline training details). \DIFdelbegin \DIFdel{Fine-tuning all weights produces superior ensemble quality compared to training only the SDL parameters while maintaining computational efficiency. See ~}\DIFdelend \DIFaddbegin \DIFadd{We note the framing of this figure explicitly. The marginal cost is the relevant number for a center that already operates a deterministic model, which is our intended use case. A like-for-like comparison against training a generative ensemble model from scratch must instead use the pretraining-plus-fine-tuning total (see \ref{sec:train_details}, where both forward-pass counts are given). See }\DIFaddend \ref{sec:train_details} for complete training details and computational cost analysis.
...
where $\epsilon := \frac{(1-\alpha)}{M}$, and $\alpha \in (0,1]$ is a hyperparameter. Setting $\alpha = 1$ recovers the fair CRPS \cite{ferro2013fair}, which adjusts for finite ensemble size. However, fair CRPS suffers from a degeneracy where all members except one match the target, leaving the remaining member unconstrained. The almost fair formulation with $\alpha < 1$ \DIFdelbegin \DIFdel{avoids }\DIFdelend \DIFaddbegin \DIFadd{can mitigate }\DIFaddend this pathology while maintaining near-fair scoring properties. We use $\alpha = 0.95$ following \cite{lang2024aifs}. The loss is averaged across all variables, vertical levels, and spatial locations with latitude weighting following \cite{schreck2025}.

Training uses the same 1979-2017 ERA5 data as the deterministic model, with validation performed on 2018-2019 and testing on 00Z initializations from 2022. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{As a route to adding ensemble capability to an existing deterministic model, this }\DIFaddend fine-tuning strategy is \DIFdelbegin \DIFdel{substantially more efficient than diffusion-based ensemble generation, which requires iterative denoising steps at inference and prohibitively expensive training on full trajectory sequences. Our approach incurs computational overhead only proportional to the desired ensemble size. }\DIFdelend \DIFaddbegin \DIFadd{inexpensive. No separate generative model is trained and no distillation stage is required. Inference cost is proportional to ensemble size, with each member generated in a single forward pass of the backbone. On step count alone this is comparable to well-distilled one-step consistency models \mbox{%DIFAUXCMD
\cite{stock2025swift} }\hskip0pt%DIFAUXCMD
rather than dramatically cheaper. The distinction is that the ensemble capability here is obtained directly by fine-tuning the existing deterministic backbone. }\DIFaddend Generating 50 members requires 50 forward passes with different $\mathbf{Z}$ samples but no architectural modifications or iterative refinement. The pretrained base model ensures that deterministic forecast quality is preserved without training from scratch, while the learned SDL parameters add controlled stochasticity appropriate to each hierarchical level.
...
Figure~\ref{fig:pressure_levels} shows calibration diagnostics for key pressure levels. The spread-skill ratio (SSR) quantifies ensemble calibration by comparing ensemble spread (standard deviation) to ensemble mean RMSE\DIFdelbegin \DIFdel{; well-calibrated }\DIFdelend \DIFaddbegin \DIFadd{. Well-calibrated }\DIFaddend ensembles exhibit SSR near 1.0. For all evaluated variables, SSR values start below unity at short lead times but converge toward optimal calibration (SSR $\approx$ 1.0) by day 5-10 and maintain well-calibrated spread through day 15. Specific humidity (Q850) demonstrates particularly strong calibration throughout the forecast window.
...
\DIFaddbegin \label{sec:reproducibility}
\DIFaddend 

Operational forecast centers require the ability to exactly reproduce disseminated
ensemble members for verification, case studies, and audit trails. \DIFdelbegin \DIFdel{While stochastic perturbation methods can reproduce prior forecasts by storing random number generator (RNG) seeds, this approach only enables rigid replay. In contrast, SDL 's latent tensor storage enables structural reproducibility, where the stored representation can be mathematically manipulated (e.g., rescaled or interpolated) post-inference. This is a capability unavailable with simple RNG seeds. SDL }\DIFdelend \DIFaddbegin \DIFadd{SDL }\DIFaddend generates each ensemble member from three independent latent tensors $\mathbf{Z}_1, \mathbf{Z}_2, \mathbf{Z}_3 \sim \mathcal{N}(0, \mathbf{I})$ sampled at each decoder injection point \DIFdelbegin \DIFdel{. Rather than }\DIFdelend \DIFaddbegin \DIFadd{and step, and either archival option of Section~2.3 reproduces a member exactly: }\DIFaddend storing the raw \DIFdelbegin \DIFdel{latent vectors, we archive the computed noise perturbations $\alpha \cdot \mathbf{R}_{\text{Gauss}} \odot \mathbf{S}(\mathbf{Z}_\ell) \odot \mathbf{M}$ }\DIFdelend \DIFaddbegin \DIFadd{latents plus the RNG seed (Option~A, 5.3~kB per member per step), or storing the realized perturbations $\sigma_0 \cdot \mathbf{R}_{\text{Gauss}} \odot \mathbf{S}(\mathbf{Z}_\ell) \odot \mathbf{M}$ }\DIFaddend applied at each SDL layer \DIFdelbegin \DIFdel{during the forward pass. These stored perturbations are compact ($\sim$5~MB per member per forecast time), substantially reducing archival requirements relative to complete atmospheric state fields ($\sim$35~MB per member per time). Crucially, this representation supports post-inference manipulation through mathematical operations such as scaling or interpolation. This is a capability that distinguishes SDL from methods that discard noise after inference. }\DIFdelend \DIFaddbegin \DIFadd{(Option~B, self-contained but larger than the forecast fields). We verified both directly. Over a 5-step (30\,h) rollout on a single GPU, replay from stored perturbations and replay from latents plus seed each reproduce the original member bit-exactly (maximum absolute difference of exactly zero at every step). Reproduction across hardware configurations is bounded rather than bitwise. Regenerating the same member on CPU differs from the GPU original by a maximum absolute difference of $2.7\times10^{-2}$ in the affected prognostic field, attributable to differing floating-point reduction orders between GPU and CPU kernel implementations, so exact replay should be performed on the archiving configuration. What distinguishes this representation from storing an RNG seed alone is not size but structure. Because the stored representation is indexed by injection scale, it can be mathematically manipulated post-inference (rescaled per scale, or interpolated between members), whereas a seed permits only rigid replay. We note that comparably compact low-dimensional conditioning noise, as in \mbox{%DIFAUXCMD
\cite{alet2024fgn}}\hskip0pt%DIFAUXCMD
, is equally cheap to archive. The distinction we draw is the scale index, not the byte count. Post-hoc manipulation under either option requires re-running the decoder from the stored representation, at 23\% of the cost of a full forward pass per member per step (52~ms versus 228~ms on an NVIDIA A100 at batch size 1). The encoder pass and the latent draw are not repeated, and the full ensemble fields need never have been written to disk.
}\DIFaddend 


\DIFdelbegin %DIFDELCMD < \begin{figure}[ht]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{reproducibility.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Original ensemble member (left), regenerated forecast using stored latent tensor Z (center), and difference field (right) demonstrate exact reproduction and post-inference manipulation capability. The difference field shows maximum
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:reproducibility}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Multi-Scale Uncertainty Control}
...
\begin{figure}[htbp]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{latent_scaling_big_study_anomoly.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{latent_scaling_beta0_anomaly.png}
\DIFaddendFL \caption{Z500 anomalies from \DIFdelbeginFL \DIFdelFL{baseline configuration }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the deterministic forecast }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{$\beta=[1,1,1]$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\beta=[0,0,0]$}\DIFaddendFL , member 0) demonstrating hierarchical control through single-parameter latent scaling. Columns represent systematic variation of $\beta_1$ (left), $\beta_2$ (center), and $\beta_3$ (right) across the range $[-3, 3]$ while holding remaining parameters at 1.0. \DIFaddbeginFL \DIFaddFL{The deterministic reference makes positive and negative $\beta$ directly comparable on a common scale. }\DIFaddendFL Valid January 17, 2022 at 18:00 UTC (108h lead time). \DIFaddbeginFL \DIFaddFL{The same analysis for Q700 and MSLP is provided in the Supporting Information.}\DIFaddendFL }
\label{fig:multiscale_control}
\end{figure}
...
We investigate whether SDL learns scale-dependent uncertainty decomposition aligned with the architectural hierarchy described in Table~\ref{tab:decoder_scales} through systematic manipulation of individual injection point contributions. During training, latent tensors are sampled from the standard normal distribution $\mathbf{Z} \sim \mathcal{N}(0, \mathbf{I})$ without additional scaling, which we denote as the $\beta = 1$ configuration. This establishes the nominal ensemble spread learned through CRPS optimization. \DIFaddbegin \DIFadd{In this and the following sections, $\mathbf{Z}_\ell$ denotes the stored per-layer perturbation representation of a member (Section~\ref{sec:reproducibility}). Operations written on $\mathbf{Z}_\ell$ act on these stored tensors. }\DIFaddend At inference, we \DIFdelbegin \DIFdel{can manipulate this }\DIFdelend \DIFaddbegin \DIFadd{manipulate the }\DIFaddend learned uncertainty structure by applying layer-specific scaling $\mathbf{Z}_\beta = \{\beta_1\mathbf{Z}_1, \beta_2\mathbf{Z}_2, \beta_3\mathbf{Z}_3\}$, where $\beta_1, \beta_2, \beta_3$ are independent \DIFdelbegin \DIFdel{scaling factors influencing the }\DIFdelend \DIFaddbegin \DIFadd{gains on the perturbations injected at the }\DIFaddend coarsest (Level 1), intermediate (Level 2), and finest (Level 3) \DIFdelbegin \DIFdel{injection }\DIFdelend points respectively. \DIFaddbegin \DIFadd{Because the gain multiplies the injected perturbation, $\beta = 0$ at every site recovers the deterministic forecast exactly. }\DIFaddend This formulation enables targeted amplification or suppression of perturbations at specific meteorological scales. To isolate individual layer contributions, we systematically vary one parameter across the range $[-3.0, 3.0]$ while holding remaining parameters fixed at their nominal training value of 1.0.
...
We analyze a global forecast valid January 17, 2022 at 18:00 UTC (108-hour lead time), corresponding to the mature phase of Winter Storm Izzy. For each parameter configuration, we generate ensemble member 0 using identical baseline noise but scaled \DIFdelbegin \DIFdel{latent tensors}\DIFdelend \DIFaddbegin \DIFadd{stored perturbations}\DIFaddend , computing Z500 anomalies relative to the \DIFdelbegin \DIFdel{baseline $\beta=[1,1,1]$ configuration}\DIFdelend \DIFaddbegin \DIFadd{deterministic $\beta=[0,0,0]$ forecast so that positive and negative $\beta$ are directly comparable on a common scale}\DIFaddend . Figure~\ref{fig:multiscale_control} presents the systematic single-parameter exploration across 21 configurations spanning seven values per parameter. The anomaly framework isolates the \DIFdelbegin \DIFdel{direct meteorological impact of latent scaling }\DIFdelend \DIFaddbegin \DIFadd{total meteorological contribution of the noise injection at each configuration }\DIFaddend by removing the \DIFdelbegin \DIFdel{dominant }\DIFdelend \DIFaddbegin \DIFadd{deterministic }\DIFaddend synoptic pattern common to all \DIFdelbegin \DIFdel{configurations. }\DIFdelend \DIFaddbegin \DIFadd{of them. Note that because the decoder is nonlinear, the $-\beta$ and $+\beta$ panels are not point-wise negations of one another for any individual forecast. The sign symmetry demonstrated in Section~5.7 is a property of the distribution over members, not of single fields.
}\DIFaddend 

Varying $\beta_1$ (left column of Figure~\ref{fig:multiscale_control}) while holding $\beta_2 = \beta_3 = 1.0$ reveals the coarsest layer's control over large-scale organization at the synoptic/meso-$\alpha$ boundary (Table~\ref{tab:decoder_scales}, Level~1: 800~km effective resolution). Z500 anomalies exhibit large-amplitude, spatially coherent patterns with magnitudes exceeding $\pm$600~m concentrated in mid-latitudes. Kinetic energy spectra (Figure~\ref{fig:ke_spectra}b) confirm that $\beta_1$ variations primarily modulate energy at synoptic scales and the large-scale end of the meso-$\alpha$ range (wavenumbers $n=2$--10), with the inset panel revealing complex, wavenumber-dependent energy redistribution\DIFdelbegin \DIFdel{: at }\DIFdelend \DIFaddbegin \DIFadd{. At }\DIFaddend $n=1$, $\beta=-3$ maximally increases KE\DIFdelbegin \DIFdel{; conversely}\DIFdelend \DIFaddbegin \DIFadd{. Conversely}\DIFaddend , at the spectral peak ($n \approx 4$), $\beta=3$ generates the largest KE amplification. This wavenumber-dependent response demonstrates that coarse-layer perturbations redistribute energy across synoptic and large meso-$\alpha$ scales rather than applying uniform scaling.
...
\toprule
\textbf{\DIFaddFL{Field}} & \textbf{\DIFaddFL{Configuration}} & \textbf{\DIFaddFL{RMSE}} & \textbf{\DIFaddFL{Spread}} & \textbf{\DIFaddFL{SSR}} & \textbf{\DIFaddFL{CRPS}} \\
\midrule
\DIFaddFL{Z500 (m$^2$\,s$^{-2}$) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{304.1 }& \DIFaddFL{266.7 }& \DIFaddFL{0.88 }& \DIFaddFL{135.0 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{304.5 }& \DIFaddFL{245.5 }& \DIFaddFL{0.81 }& \DIFaddFL{133.8 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{335.0 }& \DIFaddFL{183.9 }& \DIFaddFL{0.55 }& \DIFaddFL{152.3 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{304.2 }& \DIFaddFL{258.9 }& \DIFaddFL{0.85 }& \DIFaddFL{135.5 }\\
\midrule
\DIFaddFL{U700 (m\,s$^{-1}$) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{3.51 }& \DIFaddFL{3.20 }& \DIFaddFL{0.91 }& \DIFaddFL{1.84 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{3.52 }& \DIFaddFL{3.03 }& \DIFaddFL{0.86 }& \DIFaddFL{1.85 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{3.76 }& \DIFaddFL{2.32 }& \DIFaddFL{0.62 }& \DIFaddFL{2.05 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{3.50 }& \DIFaddFL{3.04 }& \DIFaddFL{0.87 }& \DIFaddFL{1.83 }\\
\midrule
\DIFaddFL{V850 (m\,s$^{-1}$) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{3.07 }& \DIFaddFL{2.97 }& \DIFaddFL{0.97 }& \DIFaddFL{1.46 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{3.06 }& \DIFaddFL{2.78 }& \DIFaddFL{0.91 }& \DIFaddFL{1.47 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{3.29 }& \DIFaddFL{2.15 }& \DIFaddFL{0.65 }& \DIFaddFL{1.66 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{3.04 }& \DIFaddFL{2.82 }& \DIFaddFL{0.93 }& \DIFaddFL{1.46 }\\
\midrule
\DIFaddFL{T850 (K) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{1.54 }& \DIFaddFL{1.23 }& \DIFaddFL{0.80 }& \DIFaddFL{0.74 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{1.55 }& \DIFaddFL{1.14 }& \DIFaddFL{0.74 }& \DIFaddFL{0.75 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{1.66 }& \DIFaddFL{0.86 }& \DIFaddFL{0.52 }& \DIFaddFL{0.84 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{1.54 }& \DIFaddFL{1.19 }& \DIFaddFL{0.77 }& \DIFaddFL{0.74 }\\
\midrule
\DIFaddFL{Q850 (kg\,kg$^{-1}$) }& \DIFaddFL{control $[1,1,1]$ }& \DIFaddFL{1.01e-3 }& \DIFaddFL{9.66e-4 }& \DIFaddFL{0.96 }& \DIFaddFL{5.00e-4 }\\
 & \DIFaddFL{$\beta_1$ elim.\ $[0,1,1]$ }& \DIFaddFL{1.02e-3 }& \DIFaddFL{9.31e-4 }& \DIFaddFL{0.92 }& \DIFaddFL{5.04e-4 }\\
 & \DIFaddFL{$\beta_2$ elim.\ $[1,0,1]$ }& \DIFaddFL{1.09e-3 }& \DIFaddFL{7.25e-4 }& \DIFaddFL{0.66 }& \DIFaddFL{5.65e-4 }\\
 & \DIFaddFL{$\beta_3$ elim.\ $[1,1,0]$ }& \DIFaddFL{1.01e-3 }& \DIFaddFL{9.24e-4 }& \DIFaddFL{0.91 }& \DIFaddFL{5.02e-4 }\\
\bottomrule
\end{tabular}
\caption{\DIFaddFL{Single-scale elimination at T+108\,h for the Winter Storm Izzy case (2022-01-13 00Z initialization), for the same variable set as Figure~\ref{fig:pressure_levels}: ensemble-mean RMSE, ensemble spread, spread-skill ratio, and CRPS from 100-member ensembles in which one injection point's gain is set to zero at inference while independent noise is drawn at the remaining sites, scored identically to Figure~\ref{fig:pressure_levels}. The control row is the paper's own 100-member ensemble for this initialization. Eliminating the Level-2 (meso-$\alpha$) site produces the lowest SSR and the worst RMSE and CRPS of the three eliminations for every one of the five variables, while eliminating Level~1 or Level~3 leaves spread and skill within roughly 10\% of the control. Inference-time elimination on a single case, not a retrained ablation.}}
\label{tab:elimination}
\end{table}

\DIFadd{The retrained ablations answer the complementary, training-time question. Which design choices matter when the model is fine-tuned without them? We fine-tuned two further variants from the same deterministic base checkpoint, under the identical loss, seed, and 20-epoch schedule as the published model: full encoder-decoder injection, and SDL parameters alone with a frozen backbone. We also refit the published configuration itself from the same base as a reproducibility control. It matches the published model's scores to within 0.4--2.7\% across all metrics, variables, and lead times, so run-to-run variability sits near 1--4\%. All variants were evaluated as 20-member ensembles over 12 monthly 2022 initializations at 24, 120, and 240\,h leads for Z500, T500, U500, V500, and Q500 (Supporting Information Table~S1). Three results follow. First, the encoder-decoder variant matches the reproducibility control to within the variability floor on RMSE and CRPS while reducing ensemble spread slightly (2.9--3.8\%). The additional encoder injection sites add parameters and compute without a calibration benefit, consistent with the decoder-only design. Second, freezing the backbone degrades every probabilistic measure well beyond the floor. Relative to the published model, spread falls by 7.5--16.8\%, the spread-skill ratio by 9.5--19.8\%, and CRPS worsens by 5.1--9.2\%. Thus joint fine-tuning of all weights is not merely the configuration we used but a requirement for calibrated spread in this framework. Finally, a matched pair of fine-tunes identical except for correlated versus independent latent noise differs by at most 1.6\% and typically under 1\%, with inconsistent sign across lead times (Table~S1), below the variability floor and supporting the independent-latent default.
}

\DIFaddend % ### Start the old figure showing ensemble mean (move to SI, or remove?)
...
Figure~\ref{fig:latent_interp} shows ten interpolated forecasts for Winter Storm Izzy at 6.5-day lead time. At $e{=}0.00$, the forecast shows a broad trough over the central United States with a weak coastal disturbance. At $e{=}1.00$, the forecast shows a mature extratropical cyclone positioned offshore. The intermediate forecasts transition smoothly. The western trough weakens and shifts east. The coastal system intensifies and migrates northeast. Pressure gradients tighten as the offshore low deepens. Storm structure, trough orientation, and circulation patterns adjust systematically. \DIFdelbegin \DIFdel{Linear }\DIFdelend \DIFaddbegin \DIFadd{Empirically, linear }\DIFaddend paths through SDL's \DIFdelbegin \DIFdel{latent space }\DIFdelend \DIFaddbegin \DIFadd{stored representation }\DIFaddend produce dynamically consistent meteorological transformations \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{for this case. Consistent with the variance argument above, midpoint forecasts appear as intermediate rather than novel states.
}\DIFaddend 
...
Multi-scale perturbation experiments (Section~\ref{sec:multi_scale_control}) demonstrate that individual SDL injection points exert independent control over uncertainty at different spatial scales, enabling targeted adjustments of synoptic versus mesoscale spread. While this hierarchical control provides scientific insight into learned uncertainty structures, operational recalibration workflows benefit from simpler parameterizations. Uniform scaling, where $\beta_1 = \beta_2 = \beta_3 = \beta$, reduces the adjustment space to a single interpretable parameter representing overall ensemble dispersion. Given \DIFdelbegin \DIFdel{stored latent tensor }\DIFdelend \DIFaddbegin \DIFadd{the stored representation }\DIFaddend $\mathbf{Z}$ \DIFaddbegin \DIFadd{of a member (Section~\ref{sec:reproducibility})}\DIFaddend , applying $\mathbf{Z}_{\beta} = \beta \mathbf{Z}$ uniformly across all SDL injection points enables post-inference spread adjustment \DIFdelbegin \DIFdel{without forecast regeneration}\DIFdelend \DIFaddbegin \DIFadd{by re-running only the decoder, without re-drawing noise, re-running the encoder, or archiving the full ensemble fields}\DIFaddend .

Figure~\ref{fig:spread_control} quantifies the relationship between uniform latent scaling and ensemble spread for Winter Storm Izzy at 108-hour lead time. Five atmospheric variables demonstrate systematic spread modulation across $\beta \in [-3, 3]$, with uncertainty maxima concentrating in baroclinic zones, jet streams, and cyclone centers. The symmetric behavior with respect to $\beta = 0$ is clear. At the training distribution reference ($\beta = 1$), spread patterns align with dynamically expected structures and medium-range forecast error climatology. \DIFaddbegin \DIFadd{The perturbations selected by the afCRPS objective concentrate where the flow is genuinely uncertain, producing spread that tracks features visible in the ensemble mean. As $|\beta|$ grows, the amplitude within each scale band is scaled uniformly, so this flow-dependent structure is progressively dominated by the flow-independent spatial covariance of the injection basis itself, producing the broader, smoother spread patterns at $|\beta| = 3$. }\DIFaddend Both positive and negative amplification \DIFdelbegin \DIFdel{($|\beta| = 3$) extend spread uniformly }\DIFdelend \DIFaddbegin \DIFadd{extend spread }\DIFaddend while maintaining spatial organization\DIFdelbegin \DIFdel{and physical realism}\DIFdelend \DIFaddbegin \DIFadd{, but at the cost of moving away from the calibrated optimum (Figure~\ref{fig:latent_scaling_metrics})}\DIFaddend .
...
Ensemble mean fields (black contours) vary systematically with $\beta$ scaling while preserving overall synoptic structure and demonstrating symmetric behavior with respect to $\beta$ = 0. \DIFdelbegin \DIFdel{Contour patterns }\DIFdelend \DIFaddbegin \DIFadd{Ensemble statistics }\DIFaddend at $\beta$ = -3 and $\beta$ = +3 exhibit similarity, as do configurations at $\beta$ = -1 and $\beta$ = +1, confirming that uniform latent scaling modulates both spread and ensemble central tendency in a \DIFdelbegin \DIFdel{geometrically }\DIFdelend \DIFaddbegin \DIFadd{statistically }\DIFaddend symmetric manner. \DIFaddbegin \DIFadd{Individual member fields are not point-wise negations under $\beta \to -\beta$ because the decoder is nonlinear, but the distribution over members is invariant because the noise prior is zero-mean and symmetric. }\DIFaddend The preservation of dynamically consistent spatial patterns across the full range $|\beta| \leq 3$, where spread amplitudes deviate substantially from training distribution values, suggests that SDL's learned perturbation structure captures fundamental uncertainty relationships rather than merely fitting statistical properties of the training distribution. Quantitative verification of this symmetric scaling behavior is presented in Figure~\ref{fig:latent_scaling_metrics}.
...
\DIFaddbegin \DIFadd{A related structural limitation concerns multimodality. SDL members are generated as perturbations about a single deterministic trajectory. The mechanism can represent flow-dependent, scale-structured spread that is non-Gaussian in the output fields, but it contains no mechanism for discrete scenario separation, such as two distinct cyclone tracks or cyclogenesis versus its absence, because there is no learned distribution over alternative atmospheric states from which to draw. Fully generative approaches such as diffusion models learn a joint distribution over forecast fields and can represent such bimodal outcomes directly \mbox{%DIFAUXCMD
\cite{price2025gencast}}\hskip0pt%DIFAUXCMD
. This limitation is an aspect of the same constraint as the tail-risk issue above. Both stem from the perturbation structure being anchored to a single deterministic solution.
}

\DIFaddend Additionally, the 16-level vertical discretization optimized for lower-tropospheric resolution limits representation of stratospheric processes relevant to medium-range prediction. Sudden stratospheric warmings, stratosphere-troposphere coupling during blocking events, and quasi-biennial oscillation teleconnections may be inadequately captured in upper levels where vertical resolution is coarsest. Applications requiring explicit stratospheric dynamics would benefit from expanded vertical discretization or hybrid approaches combining fine tropospheric resolution with targeted stratospheric sampling.
...
The learned nature of SDL has both advantages and limitations compared to theory-driven approaches. SDL can capture systematic model errors and complex uncertainty patterns that emerge from the full modeling system without requiring explicit specification. However, it lacks interpretability in terms of physical processes and cannot generalize beyond training distribution bounds. \DIFaddbegin \DIFadd{We distinguish this from the scale interpretability claimed elsewhere in this paper. The uncertainty representation is interpretable in terms of spatial scale, since each latent has a known band of influence, but the perturbations are not interpretable as specific physical error sources such as convective or boundary-layer parameterization error. }\DIFaddend These approaches are complementary rather than competing. The learned perturbations could augment physics-based stochastic schemes by capturing emergent uncertainty patterns while retaining interpretable physical perturbations for fundamental atmospheric processes.

While SDL shares computational efficiency with other CRPS-trained ensemble methods, its hierarchical latent parameterization distinguishes it through explicit control mechanisms. The learned latent space enables targeted scale-dependent perturbation and post-inference spread adjustment—capabilities that provide operational flexibility beyond what is achievable with fixed stochastic injection schemes. \DIFaddbegin \DIFadd{An alternative route to spectral control is through the loss function. \mbox{%DIFAUXCMD
\citeA{lang2025multiscale} }\hskip0pt%DIFAUXCMD
constrain the spectra of probabilistic forecasts with a multi-scale proper-score formulation. The two approaches are complementary rather than competing. A spectrally-aware loss constrains the marginal spectrum of the forecast but leaves the generative mechanism free, so the spectral content of any individual perturbation is emergent and not separately addressable at inference. SDL constrains the perturbation basis itself, so each latent has an a priori known band of influence, which is what makes $\beta$ a per-scale control. A common quantitative point of comparison between the two is the kinetic energy spectrum of the perturbations (Figure~\ref{fig:ke_spectra}). We note in this connection that the CRPS objective used here is computed pointwise and averaged, so it contains no term that couples spatial locations. Whatever spatial coherence the perturbations possess derives from the decoder's convolutional structure rather than from the training objective. The slight small-scale smoothness of individual members documented in Section~\ref{sec:multivariate} is the expected consequence, superimposed on the reduced effective resolution common to machine-learning weather models generally \mbox{%DIFAUXCMD
\cite{bonavita2024limitations}}\hskip0pt%DIFAUXCMD
. A natural extension is therefore an auxiliary spatially aware proper score during fine-tuning, such as a variogram-based term \mbox{%DIFAUXCMD
\cite{scheuerer2015variogram} }\hskip0pt%DIFAUXCMD
or the multi-scale formulation of \mbox{%DIFAUXCMD
\citeA{lang2025multiscale}}\hskip0pt%DIFAUXCMD
, combining loss-based control of spatial dependence with SDL's post-hoc scale controllability. We leave this to future work.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{We expect the approach to transfer beyond U-Net-like backbones, with one qualification. The essential requirement is not the skip-connection structure as such but a hierarchy of internal representations at distinct spatial resolutions. It is the resolution of the injection site, not the latent, that sets the spatial band a perturbation occupies (Table~\ref{tab:decoder_scales}). Any encoder-decoder backbone with multi-resolution feature maps (U-Net, U-ViT, hierarchical or Swin-style transformers) therefore supports SDL directly. For a flat, single-resolution transformer the scale separation would need to be supplied by the injection itself, for example by projecting the latent through a band-limited basis before it enters the residual stream. For graph-based backbones such as GraphCast or AIFS \mbox{%DIFAUXCMD
\cite{Lam2023graphcast, lang2024aifs}}\hskip0pt%DIFAUXCMD
, the natural analogue is injection at each level of the multi-mesh, with mesh refinement level playing the role of decoder resolution. We have tested only the encoder-decoder case. The latter two are reasoned expectations.
}
...
First, training efficiency through transfer learning: fine-tuning from pre-trained deterministic weights requires approximately 1.61\% of the computational cost needed to train the baseline model from scratch\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, the relevant marginal figure for a center that already operates a deterministic model. A like-for-like comparison against from-scratch generative training must use the pretraining-plus-fine-tuning total (Appendix~A). }\DIFaddend Although all model weights undergo joint optimization during fine-tuning, the pre-trained initialization dramatically accelerates convergence compared to random initialization. Inference exhibits linear scaling with ensemble size, matching CRPS-trained alternatives\DIFdelbegin \DIFdel{while avoiding the iterative refinement overhead of diffusion methods}\DIFdelend \DIFaddbegin \DIFadd{, with each member generated in a single forward pass and no separate generative model or distillation stage required}\DIFaddend .
...
Third, controllable uncertainty quantification: the explicit latent parameterization enables \DIFdelbegin \DIFdel{perfect reproducibility through compact latent storage}\DIFdelend \DIFaddbegin \DIFadd{exact regeneration from kilobyte-scale stored representations}\DIFaddend , post-inference spread adjustment \DIFdelbegin \DIFdel{via latent rescaling}\DIFdelend \DIFaddbegin \DIFadd{at the cost of re-running only the decoder}\DIFaddend , and systematic scenario exploration through latent interpolation. The learned latent space exhibits symmetric properties where opposite-sign perturbation scaling produces equivalent ensemble dispersion.
...
SDL \DIFdelbegin \DIFdel{demonstrates }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend that hierarchical stochastic decomposition provides effective controllable generation for atmospheric prediction, offering a parameter-efficient \DIFdelbegin \DIFdel{alternative to diffusion-based methods }\DIFdelend \DIFaddbegin \DIFadd{route to ensemble capability for existing deterministic models }\DIFaddend while enabling post-inference control over ensemble spread and \DIFaddbegin \DIFadd{exact }\DIFaddend reproducibility through compact latent storage.
...
\DIFdelbegin \DIFdel{All forecast verification in this study uses a common }\DIFdelend \DIFaddbegin \DIFadd{SDL-WXFormer forecasts are produced and verified natively at }\DIFaddend 192×288 \DIFaddbegin \DIFadd{resolution, with ERA5 verification fields on the same }\DIFaddend grid. For \DIFdelbegin \DIFdel{ensemble forecast evaluation }\DIFdelend \DIFaddbegin \DIFadd{the ensemble comparison against IFS-ENS }\DIFaddend (Figures 2–3), IFS-ENS \DIFdelbegin \DIFdel{forecasts were bilinearly interpolated from the }\DIFdelend \DIFaddbegin \DIFadd{is verified on its }\DIFaddend WeatherBench 2 distribution grid (240×121 equiangular)\DIFdelbegin \DIFdel{to the 192×288 regular latitude-longitude verification grid. }\DIFdelend \DIFaddbegin \DIFadd{, with }\DIFaddend ERA5 verification fields \DIFdelbegin \DIFdel{were similarly interpolated to 192×288 from their native resolution. SDL-WXFormer forecasts are produced natively at 192×288 resolution and require no horizontal interpolation. }\DIFdelend \DIFaddbegin \DIFadd{bilinearly regridded to that grid (xESMF, periodic in longitude). IFS-ENS member fields themselves are not regridded. Each system is thus verified on its own native grid against consistently processed ERA5 fields.
}\DIFaddend 
...
Forecast skill metrics (RMSE, MAE, ACC) are computed on the 192×288 verification grid using cosine-latitude weights normalized such that $\sum_{j} w_j = N_{\text{lat}}$ to account for meridional convergence. Ensemble metrics (CRPS, spread-skill ratio, rank histograms) use the \DIFdelbegin \DIFdel{almost-fair CRPS formulation (Equation 2) with $\alpha = 0.95$}\DIFdelend \DIFaddbegin \DIFadd{standard kernel CRPS estimator}\DIFaddend . The 2022 test period comprises 365 forecast initializations at 00Z\DIFaddbegin \DIFadd{, evaluated with 100 SDL-WXFormer members and 50 IFS-ENS members}\DIFaddend .
