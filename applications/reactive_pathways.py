...
Usage:
    python identify_reactive_trajectories.py ffs.yml
"""
...
def main():
    ...
    for time_range in forecast_times:
        ...
        print(f"\nGenerating {len(reactive_trajectories)} trajectory visualizations...")
        for i, traj in enumerate(reactive_trajectories, 1):
            output_path = reactive_dir / f"reactive_trajectory_{i:03d}.png"
            visualize_reactive_trajectory(traj, ic_dir, output_path, i)
            
            if i % 10 == 0:
                print(f"  Generated {i}/{len(reactive_trajectories)} visualizations")
