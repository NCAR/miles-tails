...
def plot_conditional_prob_distributions(df: pd.DataFrame, plot_dir: Path) -> None:
    ...
    ax.axhspan(0.25, 0.55, color='#b8d4e8', alpha=0.25, zorder=0,
               label='Typical FFS target range (0.25–0.55)')
    ax.axhline(1/np.e, color='#2166ac', lw=1.2, ls='--', alpha=0.7,
               label=r'Optimal $P = 1/e \approx 0.37$ (Kratzer et al.)')
